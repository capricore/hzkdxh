...
Site.popupBox = function(n) {
    ...
    var l = "";
    if (e.height != null) {
        l = " height:" + e.height + "px;"
    }
    var d = ["<div id='popupBoxContent", a, "' style='width:", e.width, "px;", l, "' class='popupCnBg'>", e.htmlContent, "</div>"];
    var h = Fai.top.document.documentElement.clientHeight;
    var c = Fai.top.$("body").scrollTop();
    if (c <= 0) {
        c = Fai.top.$("html").scrollTop()
    }
    if (c <= 0) {
        c = $(window).scrollTop()
    }
    var k = ["<div id='popupBox", a, "' class='formBox' style='width:", e.width, "px;left:", (Fai.top.document.documentElement.clientWidth - e.width) / 2, "px;'>", "<div class='formTLSite'><div class='formTCSite ", e.title == "" ? "formMulLanSite": "", "'>", e.title, "</div></div>", "<div class='formMSG' style='position:relative;'>", d.join(""), "</div>", "<a href='javascript:void(0);' class='formXSite popupBClose' hidefocus='true' onclick='return false;'></a>", "</div>"];
    k = k.join("");
    var j = Fai.top.$(k).appendTo("body");
    var i = (h - e.height) / 2;
    if (e.height == null) {
        i = (h - $(".popupCnBg").height()) / 2
    }
    $(j).css("top", i + c);
    var g = Fai.top.$("#popupBg" + a).outerHeight(true),
    f = Fai.top.$("#popupBoxContent" + a).outerHeight(true) + 120,
    m = 100 - (f - g) > 0 ? 100 - (f - g) : 0;
    if (g < f) {
        Fai.top.$("#popupBox" + a).css("top", m + "px")
    }
    function b(o) {
        return Object.prototype.toString.call(o) === "[object Function]"
    }
    Fai.top.$("#popupBox" + a).ready(function() {
        j.draggable({
            start: function() {
...
Site.moduleSubPanel = function(e) {
    ...
    c.that.mouseleave(function() {
        b.attr("_mouseIn", 0);
        setTimeout(d, 100)
    });
    c.that.mouseover(function() {
        b.attr("_mouseIn", 1)
    });
    if (parseInt(f.attr("_side")) === 2) {
        b.mouseleave(function() {
            Site.startFlutterInterval(f)
        });
        b.mouseenter(function() {
            Site.stopFlutterInterval(f)
        })
    }
    function d() {
...
Site.bindInTabSwitch = function() {
    $(".formTabButton").unbind("mouseenter").bind("mouseenter",
    function() {
        $(this).trigger("click")
    });
    var a = new Array();
...
Site.vote = function(f, d, c, b) {
    if (_siteDemo) {
        Fai.ing("当前为“样板网站”，请先“复制网站”再进行投票。");
        return
    }
    var e = $("#voteMsg" + f + d),
    g = parseInt($.cookie("vote" + f));
    var a = new Array();
    $("#voteItems" + f + d).find("input:checked").each(function() {
        var h = $(this).val();
        if (!isNaN(h)) {
            a.push(parseInt(h))
        }
    ...
    if (a.length == 0) {
        e.show();
        e.text(LS.voteNotSelect);
        return
    }
    e.show();
    $("#vote" + f + d).attr("disabled", true);
    $("#vote" + f + d).faiButton("disable");
    $.ajax({
        type: "post",
        url: "ajax/vote_h.jsp?cmd=voteItem",
        async: false,
        data: "vid=" + f + "&itemlist=" + Fai.encodeUrl($.toJSON(a)),
        error: function() {
            e.text(LS.voteError);
            setTimeout(function() {
                $("#vote" + f + d).removeAttr("disabled");
                $("#vote" + f + d).faiButton("enable")
            },
            3000)
        },
        success: function(i) {
            ...
            if (h.success) {
                e.text(LS.voteSuccess);
                if (c > 0) {
                    if (!$.cookie("vote" + f)) {
                        var j = new Date();
                        j.setHours(j.getHours() + c);
                        $.cookie("vote" + f, c, {
                            expires: j
                        })
                    }
...
Site.showNavSubMenu = function(mode) {
    ...
    $.each(Fai.top.$("#nav .navCenter .item"),
    function(i, n) {
        $(this).unbind();
        if (mode == 100) {
            var item = $(this);
...
Site.showNavItemContainer = function() {
    var g = Fai.top.$("#nav .navCenter");
    if (g.length > 0) {
        g.removeAttr("style")
    }
    Site.hideNavItemContainer();
    var m = Fai.top.$("#nav .itemContainer");
    var q = g.width();
    var b = 0;
    Fai.top.$.each(Fai.top.$("#nav .item"),
    function() {
        b += $(this).outerWidth(true)
    });
    Fai.top.$.each(Fai.top.$("#nav .itemSep"),
    function() {
        b += $(this).outerWidth(true)
    });
    Fai.top.$("#nav .item").hover(function() {
        var j = 0;
        Fai.top.$.each(Fai.top.$("#nav .item"),
        function() {
        ...
        if (Fai.isIE9()) {
            j += Fai.top.$("#nav .item").length
        }
        if (Fai.isIE10() || Fai.isIE11()) {
            j += 4
        }
        m.css("width", j + "px")
    },
    ...
    if (b > q) {
        if (Fai.isIE9()) {
            b += Fai.top.$("#nav .item").length
        }
        if (Fai.isIE10() || Fai.isIE11()) {
            b += 6
        }
        m.css("width", b + "px");
        var e = Fai.top.$("#nav").find(".itemPrev");
        var i = Fai.top.$("#nav").find(".itemNext");
        i.css("display", "block");
        ...
        var a = 0;
        var f;
        if (Fai.top.$(".itemSep").is(":hidden")) {
            f = 0
        } else {
            f = Fai.top.$("#nav").find(".itemSep").width() / 2
        }
        var o = Fai.top.$("#navCenter").offset().left;
        var d = [];
        Fai.top.$("#navCenter").find(".item").each(function(j) {
            d[j] = $(this).offset().left - o;
            n++
        });
        ...
        i.mousedown(function() {
            ...
            m.css("left", (m.position().left - a) + "px");
            if (Fai.top.$("#nav .itemContainer").position().left < 0) {
                e.css("display", "block")
            }
            if ((Math.abs(m.position().left) + q) >= b) {
        ...
        e.mousedown(function() {
            ...
            m.css("left", (m.position().left + a) + "px");
            if ((Math.abs(Fai.top.$("#nav .itemContainer").position().left) + q) < b) {
        ...
        if (Fai.top.$("#nav")[0].className && Fai.top.$("#nav")[0].className.indexOf("nav2") > -1) {
            var h = m.css("margin-left") ? m.css("margin-left").replace("px", "") : 0;
            var c = b + parseInt(h);
            if (Fai.isIE9()) {
                c += Fai.top.$("#nav .item").length
            } else {
                if (Fai.isIE11() || Fai.isIE10() || Fai.isMozilla()) {
                    c += 4
                }
            }
            g.css("width", c + "px");
            if (Fai.isIE11()) {
                g.css("height", m.outerHeight(true) + "px")
            }
            if (Fai.top._templateOtherStyleData.h !== parseInt(c) && Fai.top._manageMode) {
                Fai.top._templateOtherStyleData.h = parseInt(c);
                $.ajax({
                    type: "post",
                    url: "ajax/site_h.jsp?cmd=setOtherStyleData&_templateOtherStyle=" + Fai.encodeUrl($.toJSON(Fai.top._templateOtherStyleData)),
                    error: function() {},
                    success: function(j) {}
                })
            }
    ...
    function l(r, p) {
        ...
        if (j.currentStyle) {
            var s = j.currentStyle[p]
        } else {
            if (window.getComputedStyle) {
                var s = document.defaultView.getComputedStyle(j, null).getPropertyValue(p)
            }
        }
        return s
    }
...
Site.hideNavSubMenu = function() {
    Fai.top.$.each(Fai.top.$("#nav .navCenter .item"),
    function(a, b) {
        $(this).unbind()
    })
};
...
Site.initModuleNavListAcrossDisplay = function(c, d, b) {
    var a = $("#module" + c);
    m_formMiddleWidth = a.find(".formMiddle").width(),
    mSOptions = {
        moduleId: c,
    ...
    a.find(".navA_C_J .navA_J").hover(function() {
        ...
        if (k != null && k.length > 0) {
            var h = [],
            e = $("#navAcM" + c + "C" + i + "Panel");
            if (e.length == 0) {
                h.push("<div class='s_navList'>");
                $.each(k,
                function(n, m) {
                    var l = "";
                    if ( !! d[m.id]) {
                    ...
                    h.push("<table class='navAcrossCotent childNavA_J g_item' cellpadding='0' cellspacing='0' _colId=" + m.id + " style='*width:auto;'><tr>");
                    h.push("<td class='navAcrossCotentLeft'></td>");
                    h.push("<td class='navAcrossCotentCenter'>");
                    if (b && !!d[m.id]) {
                        h.push("<a href='javascript:void(0);' style='cursor:default;'>" + m.name + "</a></td>")
                    } else {
                        h.push("<a href='" + m.url + "'>" + m.name + "</a></td>")
                    }
                    h.push("<td class='navAcrossCotentRight" + l + "'></td>");
                    h.push("</tr></table>")
                });
                h.push("</div>");
                mSOptions.idStr = "navAcM" + c + "C" + i + "Panel";
                mSOptions.contentStr = h.join("");
            ...
            if (!Fai.isIE6() && !Fai.isIE7()) {
                var g = a.offset().left + a.outerWidth();
                a.css("z-index", 9030);
            ...
            e.find(".childNavA_J").hover(function() {
                ...
                if (p != null && p.length > 0) {
                    var m = [],
                    o = $("#navAcM" + c + "C" + s + "Panel");
                    if (o.length == 0) {
                        m.push("<div class='s_navList'>");
                        $.each(p,
                        function(t, u) {
                            m.push("<table class='navAcrossCotent grandNavA_J g_item' cellpadding='0' cellspacing='0' style='*width:auto;'><tr>");
                            m.push("<td class='navAcrossCotentLeft'></td>");
                            m.push("<td class='navAcrossCotentCenter'>");
                            m.push("<a href='" + u.url + "'>" + u.name + "</a>");
                            m.push("<td class='navAcrossCotentRight'></td>");
                            m.push("</tr></table>")
                        });
                        m.push("</div>");
                        mSOptions.idStr = "navAcM" + c + "C" + s + "Panel";
                        mSOptions.contentStr = m.join("");
                        o = Site.moduleSubPanel(mSOptions);
                        var r = n.offset().left,
                        q = n.offset().top;
                        if (Fai.isIE6() || Fai.isIE7()) {
                            r += n.outerWidth()
                        ...
                        if (!Fai.isIE6() && !Fai.isIE7()) {
                            var l = n.offset().left + n.outerWidth();
                            o.css("z-index", 1);
                    ...
                    o.find(".grandNavA_J").hover(function() {
                        $(this).addClass("g_hover")
                    },
                    function() {
                        $(this).removeClass("g_hover")
...
Site.initModulePhotoSwitch = function(c, e, b) {
    var d = Fai.top.$("#module" + c);
    var j = d.find(".formMiddleContent");
    d.data("photoSwitchData", e);
    d.data("photoSwitchType", b);
    if (b == 1) {
        var h = $("#photoSwitch" + c);
        h.imageSwitch(e);
        if (j.width() < h.width()) {
        ...
        var a = h.find(".imageSwitchShowName").eq(0);
        if (a.find(".spanHiddenName") && a.css("display") == "block") {
            if (a.find(".spanHiddenName").length == 1) {
                a.css("bottom", 0)
            }
        }
    } else {
        if (b == 2) {
            var f = $("#photoDotSwitch" + c);
            f.imageSwitch(e);
            var a = f.find(".imageSwitchShowName").eq(0);
            var i = f.find(".imageSwitchBtnArea").eq(0);
            var g = 0;
            i.css("left", "");
            if (i.width() > f.width()) {
                i.find(".imageSwitchBtn_dot").each(function() {
                    $(this).css("marginRight", 2 + "px")
                });
                if (i.width() > f.width()) {
                    g = 1;
                    i.css("marginLeft", "-" + i.width() / 2 + "px")
                }
            }
            if (g == 0) {
                i.css("marginLeft", "-" + i.width() / 2 + "px");
            ...
            if (a.css("display") == "block" && a.find(".spanHiddenName")) {
                if (a.find(".spanHiddenName").length == 1) {
                    a.css("bottom", 0)
                } else {
                    a.height(47);
                    a.css("top", f.height() - a.height() + "px")
                }
                a.width("100%");
...
Site.loadPhotoMarquee = function(t, D, A, m) {
    var e = Fai.top["Photo" + t].ieOpt,
    h = Fai.top["Photo" + t].tgOpt,
    n = Fai.top["Photo" + t].callbackArgs;
    var b = $("#module" + t);
    if (Fai.isNull(b)) {
        return
    }
    ...
    if (m == 0 || m == 1) {
        ...
        x = y.width();
        if (x <= a) {
            Site.imageEffect(t, e, h, Site.createImageEffectContent_photo, n);
            return
        }
    }
    if (m == 2 || m == 3) {
    ...
    s.each(function(F, G) {
        ...
        if (Fai.isIE6() || Fai.isIE7()) {
            if (F === c) {
                $(s[F]).after('<div style="clear:both;"></div>');
                E.after('<div style="clear:both;"></div>')
            }
        }
        y.append(E);
        r += E.outerWidth(true)
...
Site.loadPhotoGallery = function(u) {
    ...
    if (a.length != 0) {
        ...
        var K = c.find("a.gallery-control"),
        o = c.find("a.gallery-control-prev"),
        Q = c.find("a.gallery-control-next"),
        I = o.height(),
        d = o.width();
        var O = a.first(),
        R = O.children("div.img-container"),
        A = R.outerWidth(true),
        r = R.outerHeight(true),
        y = O.children("div.prop-container"),
        g = 0;
        if (y.length > 0) {
        ...
        w.height(B);
        var v = c.find("div.photo-gallery-inner"),
        n = v.outerHeight();
        if (n < I) {
            var L = v.height(),
            P = n - L;
        ...
        var b = 1,
        W = o.innerWidth() - d,
        C = c.find("div.formMiddleContent").width(),
        q = C - W - (d * 2),
        Y = O.outerWidth(true),
        S = O.outerWidth(),
        f = Y - S;
        ...
        o.addClass("gallery-control-prev-disabled");
        if (s == 1) {
        ...
        o.click(function() {
            ...
            var Z = x.parent(),
            j = Z.find("div.photo-gallery-container"),
            aa = Z.find("a.gallery-control-next");
        ...
        Q.click(function() {
            ...
            var Z = ae.parent(),
            i = Z.find("div.photo-gallery-container"),
            x = Z.find("a.gallery-control-prev");
            if (x.hasClass("gallery-control-prev-disabled")) {
            ...
            if (!ae.data("loading_finish")) {
                ...
                var j = ad <= 1 ? 1 * b: ad * b,
                ab = j + b > a.length ? a.length: j + b;
                for (var aa = j; aa < ab; aa++) {
                    var ac = $(a[aa]).find("img");
                    if (!ac.attr("src")) {
...
Site.loadPhotoSmallPic = function(e) {
    ...
    d = e.cus;
    var c = $("#" + m);
    c.on("Fai_onModuleSizeChange",
    function() {
        Site.smallPicPhotoModuleFix(m)
    });
    c.on("Fai_onModuleLayoutChange",
    function() {
        Site.smallPicPhotoModuleFix(m)
    });
    ...
    y.click(function() {
        ...
        var U = F.find(".photoContainerLeft").width(),
        Q = F.find(".photoContainerLeft").height();
        ...
        var C = [];
        C.push("<div class='loadingImg' style='width:" + U + "px;height:" + Q + "px;'>");
        C.push("<table cellspacing='0' cellpadding='0' class='loadingImgTable' style='width:" + U + "px;height:" + Q + "px;'><tbody><tr><td class='loadingImgTd'>");
    ...
    q.click(function() {
        ...
        var j = M.parents(".photoSmallPicDownForms").find(".photoSmallPrePicContainer"),
        F = M.parents(".photoSmallPicDownForms").find("a.g_imgPrev"),
        E = Math.floor(D.find(".photoSmallPicDownFormsMid").width() / a) + 1;
        if (F.hasClass("photoSmallPic-control-prev-disabled")) {
            F.removeClass("photoSmallPic-control-prev-disabled")
        }
        var N = (E - 1) * a,
        L = parseInt(j.css("left").replace("px"));
        var K = Math.abs(L) / a;
...
Site.multiPhoto = function(m, l, e) {
    ...
    d.mouseover(function() {
        ...
        var E = w.height();
        var z = [];
        z.push("<div class='ProductDetailloadingImg' style='width:" + v + "px;height:" + E + "px;'>");
        z.push("<table cellspacing='0' cellpadding='0' class='ProductDetailloadingImgTable' style='width:" + v + "px;height:" + E + "px;'><tbody><tr><td class='ProductDetailloadingImgTd'>");
        z.push("<div class='ajaxLoading2' style='margin:0 auto;'></div>");
        z.push("</td></tr></tbody></table>");
        z.push("</div>");
        w.append(z.join(""));
        $(y).load(function() {
            $(".ProductDetailloadingImg").remove();
            $(".multiPhotoImgLoad").fadeOut("slow",
            function() {
                $(this).remove()
            })
        });
...
Site.createImageEffectContent_photo = function(g, i, b, j) {
    var d = $(g).parents("." + b.targetParent);
    var h = $(g).find(".imageEffects");
    var c = $(d).attr("photoname");
    if (b.nameHidden || c.length == 0) {
        return
    }
    e(c);
    f(g, i, b.nameWordWrap);
    a();
    function e(k) {
        var l = [];
        l.push("<div class='props'><div class='propList g_specialClass'>" + k + "</div></div>");
        $(h).append(l)
    }
    function f(q, r, k) {
        var l = $(q).find(".imageEffects").height() / 3;
        var p = $(h).find(".props");
        var s = $(p).find(".propList");
        var o = 0;
        var m = l;
        var n = "";
        if (k) {
            n = "overflow: visible; text-overflow: clip; white-space: normal; word-wrap: break-word; "
        } else {
        ...
        if (r.effType == 5) {
            ...
            $(h).css("height", m).css("bottom", "-" + m + "px")
        }
    }
    function a() {
        if (i.effType == 5) {
        ...
        var l = $(h).height();
        var n = $(h).find(".props").height();
        var k = $(h).find(".props .propList");
        var m = (l - n) / 2;
        $(k).css("padding-top", m + "px")
    }
};
Site.clearImageEffectContent_photo = function(b, a) {
    $("#" + b).find("." + a).remove()
};
Site.bindImageEffectCusEvent_photo = function(target, tgOpt) {
    var imageEffect = $(target).find(".imageEffects");
    var isHasHref = true;
    var targetATag = $(target).parents("." + tgOpt.targetParent).find("a");
    var targetATag_href = $(targetATag).attr("href");
    var isJump = (typeof targetATag_href == "undefined" || targetATag_href.length == 0 || targetATag_href.indexOf("javascirpt") > 0 || targetATag_href.indexOf("return") > 0 || targetATag.length == 0) ? false: true;
    if (!isJump) {
        isHasHref = false
    }
    if (isHasHref) {
        ...
        $(imageEffect).css("cursor", "default")
    }
    $(imageEffect).unbind("click").bind("click",
    function() {
        var targetATagTmp = $(this).parents("." + tgOpt.targetParent).find("a");
        if (Fai.isNull(targetATagTmp)) {
            return
        }

        var _href = $(targetATagTmp).attr("href");
        var _onclick = $(targetATagTmp).attr("onclick");
        var _target = $(targetATagTmp).attr("target");
        var isJump = (typeof _href == "undefined" || _href.length == 0 || _href.indexOf("javascirpt") > 0 || _href.indexOf("return") > 0 || targetATag.length == 0) ? false: true;
...
Site.initModuleWeather2 = function(b, h, a, c) {
    var e = (a == 11 ? 2 : 1),
    g = 0,
    i = "json",
    d = Fai.isIE6(),
    f = [];
    var j = "//platform.sina.com.cn/weather/forecast?app_key=1315597423&city=" + encodeURIComponent(h) + "&startday=" + g + "&lenday=" + e + "&format=" + i + "&auth_type=uuid&auth_value=0123456789012345&callback=?";
    ...
    $.getJSON(j,
    function(q) {
        if (!q || q.status.code != 0) {
            if (!c) {
                return
            }
            Site.initModuleWeather2(b, c, a);
            return
        }
        var n = $(".formMiddleContent" + b + " .weather2"),
        v = q.data.city[0].days.day,
        u = false;
        for (var s = 0; s < v.length; s++) {
            var x = v[s].f1 ? v[s].f1: "qing",
            w = v[s].f2 ? v[s].f2: "qing",
            r = (x == w ? true: false),
            p = v[s].t1 ? v[s].t1: "0",
            m = v[s].t2 ? v[s].t2: "0",
            o = v[s].s1,
            ...
            t = v[s].d1,
            A = v[s].p1,
            y = q.data.city[0].info.name;
            n.find(".cityName" + a).text(y);
            if (a != 12 && a != 11) {
                f.push("<ul class='weather2'>");
                f.push("<li class='cityName" + a + "'>");
                f.push("" + y + "</li>");
                f.push("<li class='weatherCon" + a + "'>");
                if (a == 1) {
                    if (d) {
                    ...
                    if (a == 3 || a == 11) {
                        if (!r) {
                            f.push("<img style='margin-right: 5px;' class='image1" + a + "' title='" + o + "' src='" + _resRoot + "/image/site/weather/" + x + "1." + (d ? "gif'": "png'") + "/>")
                        }
                    }
                    f.push("<img class='image2" + a + "' title='" + l + "' src='" + _resRoot + "/image/site/weather/" + w + "1." + (d ? "gif'": "png'") + "/>")
                }
                f.push("</li>");
                f.push("<li class='temperature" + a + "'>");
                f.push("<span class='temperature1" + a + "'>");
                f.push("" + p + "</span> ~");
                f.push("<span class='temperature2" + a + "'>");
                f.push("" + m + "</span>");
                f.push("</li>");
                if (a == 7) {
                    f.push("<li class='weatherCondition" + a + "'>");
                    f.push("" + t + "</li>")
                }
                f.push("</ul>")
            } else {
                if (a == 11) {
                    $(".formMiddleContent" + b + " .includeWeather" + a).width(520);
                    if (d) {
                        $(".formMiddleContent" + b).css("height", 81 + "px")
                    }
                    var k = "";
                    s == 0 ? k = "今天": k = "明天";
                    f.push("<div class='weather2'>");
                    f.push("<div class='cityName" + a + "'>" + (!u ? y: "") + "</div>");
                    f.push("<img class='image1" + a + "' title='" + o + "' src='" + _resRoot + "/image/site/weather/" + x + "11." + (d ? "gif'": "png'") + "/>");
                    f.push("<div class='weatherToday" + a + "'>");
                    f.push("<div class='weatherTodayInfo" + a + "'>");
                    f.push("<span class='weatherTodayDate" + a + "'>" + k + " ");
                    f.push("" + p + " ~ </span>");
                    f.push("" + m + "<span class='temperature1" + a + "'>");
                    f.push("</span> ");
                    f.push("<span class='temperature2" + a + "'>");
                    f.push("</span>");
                    f.push("</div>");
                    f.push("<div class='weatherCon" + a + "'>");
                    f.push("<span class='weatherCon1" + a + "'>" + o + "</span>");
                    f.push("<span>转</span>");
                    f.push("<span class='weatherCon2" + a + "'>" + l + "</span>");
                    f.push("</div>");
                    f.push("</div>");
                    f.push("</div>");
                    u = true
                    ...
                    if (a == 12) {
                        ...
                        f.push("<div class='weather2' style='width:250px;'>");
                        f.push("<div class='left" + a + "' style='width: 90px;'>");
                        f.push("<div class='cityName" + a + "'>" + y + "</div>");
                        f.push("<div class='images" + a + "'>");
                        f.push("<img class='image1" + a + "' title='" + o + "' src='" + _resRoot + "/image/site/weather/" + x + "11." + (d ? "gif'": "png'") + "/>");
                        f.push("</div>");
                        f.push("<div class='weatherCon" + a + "'>");
                        if (!r) {
                        ...
                        f.push("</div>");
                        f.push("</div>");
                        f.push("<div class='right" + a + "'>");
                        f.push("<div class='temperatureInclude" + a + "'>");
                        f.push("<div style='float: left;'>温度：</div>");
                        f.push("<div class='temperature" + a + "'>");
                        f.push("<span class='temperature1" + a + "'>" + p + "");
                        f.push("</span> ~");
                        f.push("<span class='temperature2" + a + "'>" + m + "");
                        f.push("</span>");
                        f.push("</div>");
                        f.push("</div>");
                        f.push("<div class='windInclude" + a + "'>");
                        f.push("<div style='float: left;'>风力：</div>");
                        f.push("<div class='wind" + a + "'>");
                        f.push("" + A + "</div>");
                        f.push("</div>");
                        f.push("<div class='windDirectionInclude" + a + "'>");
                        f.push("<div style='float: left;'>风向：</div>");
                        f.push("<div class='windDirection" + a + "'>");
                        f.push("" + t + "</div>");
                        f.push("</div>");
                        f.push("</div>")
                    }
        ...
        if (f.length == 0) {
            var z = "<div id='toolTips" + a + "' style='margin: 0 auto;width: 120px; cursor:pointer;' onclick='Site.initModuleWeather2( " + b + ',"' + h + '", ' + a + ',"' + c + "\")'>网络缓慢，请重新加载</div>";
            $(".formMiddleContent" + b + " .includeWeather" + a).append(z)
        } else {
            $(".formMiddleContent" + b + " .includeWeather" + a).append(f.join(""))
        }
    ...
    jQuery.fn.extend({
        bannerImageSwitch: function(c) {
            ...
            return a(this).each(function() {
                var f = a(this);
                var C = b.width || f.width();
                var K = b.height || f.height();
                var G = b.data.length;
                var I = b.index;
                var e = C < f.width() ? C: f.width();
                var D = f.width();
                f.css("overflow", "hidden");
                f.height(K);
                var L = a('<div class="imageSwitchShowName photoSwitchBg" />').appendTo(f).css("position", "absolute").css("display", "none").css("zIndex", 2).css("width", a.browser.msie && a.browser.version == 6 ? a(f).parent().parent().width() + "px": a(f).parent().width() + "px").css("height", 30 + "px").css("line-height", 30 + "px");
                if (G == 0) {
                ...
                if (b.btnType != 0) {
                    var N = a('<div class="imageSwitchBtnArea"/>').appendTo(f).css("position", "absolute").css("zIndex", 3).css("display", F)
                }
                a("<div />").appendTo(f);
                var l = a('<div class="switchGroup"/>').appendTo(f).css("width", (b.width <= f.width()) ? b.width: "100%").css("position", "relative").height("y,show-y".indexOf(b.animateStyle) != -1 ? K * G: K).css("overflow", "hidden").css("margin", "0 auto");
                if (b.wideScreen) {
                    l.css("width", "100%")
                }
                var P = 0;
                ...
                function(T, W) {
                    ...
                    w = "";
                    m = "";
                    if (a.trim(W.edgeLeft) != "") {
                        w = "background:" + W.edgeLeft + ";"
                    }
                    if (a.trim(W.edgeRight) != "") {
                        m = "background:" + W.edgeRight + ";"
                    }
                    if (!W.href) {
                    ...
                    if (b.btnType == 0) {
                        if (b.wideScreen) {
                            ...
                            s += "left:" + ((T - I) * b.width) + "px;"
                        }
                    }
                    J.push("<div class='" + Q + "' id='" + Q + T + "' style='" + s + "'>\n");
                    g = "";
                    g += "width:100%;";
                    g += "height:" + K + "px;";
                    g += "cursor:" + (W.href ? "pointer": "default") + ";";
                    ...
                    if (W.width && W.width > 0) {
                        h = ("<img src='" + W.src + "' width='" + W.width + "' height='" + W.height + "' />\n")
                    } else {
                        h = "";
                        g += "background-image:url(" + W.src + ");"
                    }
                    if (b.wideScreen) {
                        J.push("<a class='J_bannerEdge J_bannerEdgeLeft bannerEdge bannerEdgeLeft' hidefocus='true' title='" + (W.tip ? W.tip: "") + "' href='" + (W.href ? Fai.encodeHtml(W.href) : "javascript:;") + "' target='" + (W.target ? W.target: "") + "' " + S + " style='" + w + "'></a>");
                        J.push("<a class='J_bannerEdge J_bannerEdgeRight bannerEdge bannerEdgeRight' hidefocus='true' title='" + (W.tip ? W.tip: "") + "' href='" + (W.href ? Fai.encodeHtml(W.href) : "javascript:;") + "' target='" + (W.target ? W.target: "") + "' " + S + " style='" + m + "'></a>")
                    }
                    J.push("<a class='J_bannerItem' hidefocus='true' title='" + (W.tip ? W.tip: "") + "' href='" + (W.href ? Fai.encodeHtml(W.href) : "javascript:;") + "' target='" + (W.target ? W.target: "") + "' style='" + g + "' " + S + ">\n");
                    J.push(h);
                    J.push("</a>\n");
                    J.push("</div>\n");
                    l.append(J.join(""));
                    if (b.btnType == 1) {
                        var U = a('<a class="imageSwitchBtn" />').appendTo(N).html("<span>" + (T + 1) + "</span>")
                    } else {
                        if (b.btnType == 2) {
                            var U = a('<a class="imageSwitchBtn_dot" />').appendTo(N)
                        }
                    }
                    var R = a('<span class="spanHiddenName"/>').appendTo(L).css("margin-left", 10 + "px").text(W.name);
                    if (b.btnType != 0) {
                        if (T == I) {
                            if (b.btnType == 1) {
                                U.addClass("imageSwitchBtnSel")
                            } else {
                                U.addClass("imageSwitchBtnSel_dot")
                            }
                            R.addClass("spanShowName")
                        }
                    }
                    if (b.btnType == 1) {
                        P += Fai.getDivWidth(U)
                ...
                if (b.btnType == 0) {
                    if (G > 1) {
                        var B = a('<a class="imageSwitchBtn_arrow arrow_prev" />').appendTo(f).css({
                            position: "absolute",
                            zIndex: 3
                        });
                        var M = a('<a class="imageSwitchBtn_arrow arrow_next" />').appendTo(f).css({
                            position: "absolute",
                            zIndex: 3
                        })
                    }
                        ...
                        if (Fai.top.$("#banner").length > 0) {
                            if (b.btnType == 1) {
                                if (C >= f.width()) {
                                    ...
                                    N.css("right", "50%");
                                    N.css("marginRight", "-" + C / 2 + "px")
                                }
                            } else {
                                if (b.btnType == 2) {
                                    N.css("left", "50%");
                                    N.css("marginLeft", "-" + N.width() / 2 + "px")
                                }
                            }
                        } else {
                            N.css("left", (t - P) + "px")
                ...
                if (b.btnType != 0) {
                    var E = N.children("a");
                    var v = l.find(".J_bannerItem")
                } else {
                    var d = M;
                    var x = B
                }
                var y = L.children("span");
                var n = t - P;
                if (G > 1) {
                    if (b.btnType != 0) {
                        L.css("top", (H - N.height() - 9) + "px").css("left", 0 + "px")
                    } else {
                        L.css("top", H + "px").css("left", 0 + "px")
                    }
                    var z = a.browser.msie && a.browser.version == 6 ? parseInt(j() + 30) : parseInt(j() + 20);
                    if (n > z) {
                        if (b.btnType == 1) {
                            N.css("top", (H - N.height() - 6) + "px")
                        } else {
                            if (b.btnType == 2) {
                                N.css("bottom", "5px")
                            } else {
                                B.css("top", "50%");
                                B.css("marginTop", "-" + B.height() / 2 + "px");
                                M.css("top", "50%");
                ...
                if (G > 1) {
                    if (b.btnType != 0) {
                        E.click(function(S) {
                            ...
                            if (b.btnType == 1) {
                                E.eq(I).removeClass("imageSwitchBtnSel");
                                E.eq(R).addClass("imageSwitchBtnSel")
                            } else {
                                E.eq(I).removeClass("imageSwitchBtnSel_dot");
                                E.eq(R).addClass("imageSwitchBtnSel_dot")
                            }
                            y.eq(I).removeClass("spanShowName");
                            y.eq(R).addClass("spanShowName");
                            switch (b.animateStyle) {
                            case "o":
                                v.eq(I).fadeOut(b.animateTime, "failinear");
                                v.eq(R).fadeIn(b.animateTime, "failinear");
                                if (b.wideScreen) {
                                    v.eq(I).siblings().fadeOut(b.animateTime, "failinear");
                                    v.eq(R).siblings().fadeIn(b.animateTime, "failinear")
                                }
                                break;
                            case "x":
                                l.animate({
                            ...
                            case "show-x":
                            case "show-y":
                                v.eq(I).hide(b.animateTime);
                                v.eq(R).show(b.animateTime);
                                break;
                            case "none":
                                v.eq(I).hide();
                                v.eq(R).show();
                                if (b.wideScreen) {
                                    v.eq(I).siblings().hide();
                                    v.eq(R).siblings().show()
                                }
                                break
                            }
                            I = R;
                            var T = Fai.top.$("#bannerGetHref");
                            if (T) {
                                T.css({
                                    width: b.data[R].imgWidth,
                                    left: (f.width() - b.data[R].imgWidth) / 2 + "px"
                                });
                                T.css({
                                    height: b.data[R].imgHeight,
                                    top: (f.height() - b.data[R].imgHeight) / 2 + "px"
                                })
                            }
                        });
                        function q() {
                        ...
                        function o(S) {
                            ...
                            Fai.stopInterval(O);
                            y.eq(p).removeClass("spanShowName");
                            if (p == G - 1) {
                                p = -1
                            }
                            y.eq(++p).addClass("spanShowName");
                        ...
                        x.on("click", i);
                        x.hover(function() {
                            x.addClass("arrow_prev_hover")
                        },
                        function() {
                            x.removeClass("arrow_prev_hover")
                        });
                        function A() {
                            d.click()
                        }
                    }
                    var O = "imageSwitch" + Math.random();
                    if (b.btnType != 0) {
                        Fai.addInterval(O, q, b.playTime)
                    } else {
                        Fai.addInterval(O, A, b.playTime)
                    }
                    Fai.startInterval(O);
...
Site.bannerGetHref = function() {
    ...
    a.find(".switchGroup").children().each(function(d, e) {
        if (d == c) {
            b = Fai.top.$(e).find("a").eq(0).attr("href");
            return false
        }
    });
    if (b != "javascript:;") {
...
Site.refreshBannerImageSwitch = function(h) {
    ...
    function(t, x) {
        ...
        if (x.e != 0) {
            var r = Fai.top.$("#banner .switchGroup div").eq(t).find("a").eq(0).attr("href");
...
Site.memberProfileSubmit = function(b, o) {
    ...
    $.ajax({
        ...
        data: "id=" + o + "&info=" + Fai.encodeUrl($.toJSON(f)),
        error: function() {
        ...
        success: function(q) {
            ...
            if (r.success) {
                ...
                d.find(".resetPwd").html(LS.memberProfileResetPwd)
            } else {
                if (r.rt = -3) {
...
Site.memberSignupSubmit = function(b, a, i) {
    ...
    $.ajax({
        ...
        success: function(j) {
            ...
            if (k.success) {
                if (k.active) {
                    Site.memberActiveDialog(g.info.email, Fai.encodeUrl(g.info.acct),
                    function() {
                        if (a) {
...
Site.checkSignupValid = function(q, e) {
    var b = {
        checkResult: true,
        captcha: "",
        info: {}
    ...
    var j = $.trim($("#memberSignupAcct").val());
    if (j == null || j == "") {
        Site.showSignupMsg(e, q, "memberSignupAcct", LS.memberSignupRegisterAcctEmpty);
        b.checkResult = false;
        return b
    }
    var g = $("#memberSignupPwd").val();
    if (g == null || g == "") {
        Site.showSignupMsg(e, q, "memberSignupPwd", LS.memberSignupRegisterPwdEmpty);
        b.checkResult = false;
        return b
    }
    if (g.length < 4) {
        Site.showSignupMsg(e, q, "memberSignupPwd", LS.memberSignupRegisterPwdMinLength);
        b.checkResult = false;
        return b
    }
    var l = $("#memberSignupRepwd").val();
    if (g != l) {
        Site.showSignupMsg(e, q, "memberSignupRepwd", LS.memberSignupRegisterPwdNotMatch);
        b.checkResult = false;
        return b
    }
    var m = "";
    if ($("#memberSignupRemark").length > 0) {
        m = $("#memberSignupRemark").val();
        var r = $("#memberSignupRemark").attr("maxlength");
        if (m.length > r) {
            Site.showSignupMsg(e, q, "memberSignupRemark", Fai.format(LS.memberSignupRegisterRemarkMaxLength, r));
            b.checkResult = false;
            return b
        }
    }
    ...
    $(".userAddItem").each(function() {
        ...
        h = $(this).val();
        c = $(this).attr("name");
        k = $(this).attr("maxlength");
        b.info[userAddItemID] = h;
        if (h.length > k) {
            Site.showSignupMsg(e, q, userAddItemID, Fai.format(LS.memberSignupUserAddItemMaxLength, c, k));
            p = 1;
            b.checkResult = false;
            return b
        }
        if (userAddItemID == "phone" && h.length > 0) {
            if (!Fai.isPhone(h)) {
                Site.showSignupMsg(e, q, userAddItemID, Fai.format(LS.memberSignupUserAddItemCorrect, c));
                f = 1;
                b.checkResult = false;
                return b
            }
    ...
    $(".isCheckUAI").each(function() {
        ...
        c = $(this).attr("name");
        if (h == null || h == "") {
            Site.showSignupMsg(e, q, userAddItemID, Fai.format(LS.memberSignupUserAddItemIsEmpty, c));
            i = 1;
            return false
        }
    ...
    if (b.captcha == null || b.captcha == "") {
        Site.showSignupMsg(e, q, "memberSignupCaptcha", LS.memberSignupRegisterCaptchaEmpty);
        b.checkResult = false;
        return b
    }
    var o = $("#memberAgreePro");
    if (o.length > 0) {
        if (!o.prop("checked")) {
            Site.showSignupMsg(e, q, "memberAgreePro", LS.memberProtocolNotAgree, true);
            b.checkResult = false;
            return b
        }
    }
...
Site.initModuleMemberLogin = function(a, c, d, b) {
    if (c == 2) {
        if (d) {
            $("#module" + a).find(".J_loginButton").bind("click",
            function() {
                if (b) {
...
Site.memberLogin = function(a) {
    Fai.top.location.href = "login.jsp?url=" + Fai.encodeUrl(Fai.getUrlRoot(Fai.top.location.href)) + "&errno=" + a
};
Site.memberLogin1 = function(b) {
    var a = Fai.getUrlRoot(Fai.top.location.href);
    var c = $("#module" + b);
...
Site.memberLogin2 = function(b, k) {
    var a = Fai.getUrlParam(Fai.top.location.href, "url");
    if (!a) {
        a = "./index.jsp"
    }
    var c = $("#module" + b);
    var j = $.trim(c.find(".memberAcctInput").val());
    var g = c.find(".memberPwdInput").val();
    var f = (c.find(".memberCaptcha").css("display") != "none");
    var h = c.find(".memberCaptchaInput").val();
    if (j == null || j == "") {
        Site.showMemberLoginMsg(b, 1, false, k);
        return
    }
    if (g == null || g == "") {
        Site.showMemberLoginMsg(b, 2, false, k);
        return
    }
    if (f && (h == null || h == "")) {
        Site.showMemberLoginMsg(b, 3, false, k);
        return
    }
    g = $.md5(g);
    var l = c.find(".memberLoginMsg");
    l.show();
    l.find(".msgText").html(LS.memberLogining);
    var i = c.find(".memberLoginBtn");
    i.attr("disabled", true);
    var e = false;
    var d = c.find("#autoLogin");
    if (d && d.attr("checked")) {
        e = true
    }
    $.ajax({
        type: "post",
        url: "ajax/login_h.jsp",
        data: "cmd=loginMember&acct=" + Fai.encodeUrl(j) + "&pwd=" + Fai.encodeUrl(g) + "&captcha=" + Fai.encodeUrl(h) + "&autoLogin=" + Fai.encodeUrl(e),
        error: function() {
            i.removeAttr("disabled");
            Site.showMemberLoginMsg(b, -1, k)
        },
        success: function(m) {
            i.removeAttr("disabled");
            ...
            if (m.success) {
                ...
                if (m.active) {
                    Site.memberInactiveDialog(m.mail, m.memName)
                } else {
                    Site.showMemberLoginMsg(b, m.rt, m.captcha, k)
                }
            }
        }
    })
...
Site.initWBLogin = function(f, a, c, b, d, e) {
    $("#" + e).click(function() {
        Site.wbPopupID = Fai.popupWindow({
            title: "",
            width: 800,
            height: 500,
            frameScrolling: false,
            frameSrcUrl: "https://api.weibo.com/oauth2/authorize?client_id=" + f + "&redirect_uri=" + a + "/wbLogin.jsp&response_type=code",
            closeFunc: function(g) {
                if (g) {
                    Site.wbWinListener(g, c, b, d)
                }
            }
        })
    })
};
Site.wbWinListener = function(a, d, c, e) {
    if (a.w_uid != null && a.w_uid != "") {
        ...
        $.ajax({
            ...
            data: "loginType=2&openId=" + a.w_uid,
            error: function() {
            ...
            success: function(f) {
                ...
                if (f.success) {
                    ...
                    if (f.active) {
                        ...
                        if (f.rt == -3) {
                            Site.memberOtherLoginAdd("", a.w_uid, jQuery.parseJSON(d), c, e, 2)
                        } else {
                            if (f.rt == -303) {
...
Site.initQQLogin = function(f, a, c, b, d, e) {
    if ((window.DocumentTouch && document instanceof window.DocumentTouch) || "ontouchstart" in window) {
        Site.qqLoginMobile(f, a, c, b, d, e)
    } else {
...
Site.qqLoginMobile = function(f, a, c, b, d, e) {
    $("#" + e).click(function() {
        Site.qqPopupID = Fai.popupWindow({
            ...
            height: 500,
            frameScrolling: false,
            frameSrcUrl: "https://graph.qq.com/oauth2.0/authorize?client_id=" + f + "&redirect_uri=" + a + "/qqLogin.jsp&response_type=token&scope=all",
            closeFunc: function(g) {
                if (g) {
...
Site.qqWinListenerMobile = function(e, c, b, d) {
    if (e.q_uid != null && e.q_uid != "") {
        ...
        $.ajax({
            type: "post",
            url: "ajax/login_h.jsp?cmd=otherLoginMember",
            data: "loginType=1&openId=" + e.q_uid,
            error: function() {
                Fai.ing(LS.memberLoginError, true)
            },
            success: function(f) {
...
Site.qqLoginPC = function(f, a, c, b, d, e) {
    $("#" + e).click(function() {
        if (Site.qqListener.c == 0) {
            ...
            Site.qqListener.timer = window.setInterval("Site.qqWinListenerPC('" + c + "', " + b + ", " + d + ")", 500); ++Site.qqListener.c
        }
    })
};
    ...
    if (Site.qqListener.winOpen.closed == true) {
        ...
        if (Site.qqListener.q_uid != null && Site.qqListener.q_uid != "") {
            ...
            $.ajax({
                type: "post",
                url: "ajax/login_h.jsp?cmd=otherLoginMember",
                data: "loginType=1&openId=" + Site.qqListener.q_uid,
                error: function() {
                    Fai.ing(LS.memberLoginError, true)
                },
                success: function(e) {
...
Site.memberOtherLoginAdd = function(e, a, f, b, c, d) {
    ...
    var i = ['<div style="padding:6px 2px;">', '<div style="padding:5px 0;">', '<div style="width:160px;float:left;">&nbsp;</div>', '<input id="bindAcct" type="checkbox" style="vertical-align:middle;"/><label for="bindAcct" style="vertical-align:middle;font-size:14px;">', LS.memberOtherLgnBindAcct, "</label>", "</div>", '<table cellpadding="0" cellspacing="0" style="_margin:0 auto">', '<tr">', '<td style="width:160px;_width:auto;text-align:right;">', LS.memberOtherLgnAcct, '</td><td><input id="acct" type="text" maxlength="50" value="', e, '" style="width:154px;"/><span style="color:red;">*</span><input id="openId" type="password" disabled style="display:none;"/></td>', "</tr>", "<tr>", '<td style="text-align:right;">', LS.memberOtherLgnPwd, '</td><td><input id="pwd" type="password" maxlength="20" style="width:154px;"/><span style="color:red;">*</span></td>', "</tr>", '<tr">', '<td style="text-align:right;">', LS.memberOtherLgnRePwd, '</td><td><input id="repwd" type="password" maxlength="20" style="width:154px;"/><span style="color:red;">*</span></td>', "</tr>"];
    $.each(f,
    function(k, m) {
        var l = ['<tr class="bindAcctHidn">', '<td style="text-align:right;">', m.name, '：</td><td><input id="', m.fieldKey, '" type="text" name="', m.name, '" maxlength="50" class="userAddItem', m.must ? " isCheckUAI": "", '" style="width:154px;"/>', m.must ? '<span style="color:red;">*</span></td>': "</td>", "</tr>", ];
        i.push(l.join(""))
    });
    if (b) {
        i.push('<tr class="bindAcctHidn">');
        i.push('<td style="text-align:right;">' + LS.memberOtherLgnRemark + "</td>");
        i.push('<td><textarea id="remark" maxlength="' + c + '" style="width:220px;height:52px;"></textarea></td>');
        i.push("</tr>")
    }
    var g = "";
    if (d == 1) {
    ...
    i.push("</table>");
    i.push('<div id="oLoginBindMsg" style="text-align:center;display:none;padding:10px 0;">' + g + "</div>");
    i.push('<div id="oLoginTip" style="text-align:center;padding-top:6px;color:red;"></div>');
    i.push('<div style="height:30px;line-height:30px;text-align:center;padding:5px 0;">');
    i.push('<a hidefocus="true" class="formBtn" href="javascript:;" onclick="Site.memberOtherLoginSubmit(' + h + "," + d + ');return false;">' + LS.confirm + "</a>");
    i.push("</div></div>");
    i = i.join("");
    var j = {
        boxId: h,
...
Site.memberOtherLoginSubmit = function(a, i) {
    ...
    $.ajax({
        type: "post",
        url: "ajax/member_h.jsp?" + o,
        data: "info=" + Fai.encodeUrl($.toJSON(r)) + "&loginType=" + i,
        error: function() {
            ...
            if (u.success) {
                if (u.fromBind) {
                    ...
                    if (u.active) {
                        Fai.top.$("#popupBg" + a).remove();
                        Fai.top.$("#popupBox" + a).remove();
                        Site.memberActiveDialog(r.email, Fai.encodeUrl(r.acct),
                        function() {
                            Fai.top.location.href = b;
                            Fai.top.event.returnValue = false
                        })
...
Site.memberFdPwdStepOne = function(c) {
    ...
    var e = ["<div class='memberFdPwdStepOne' style='height:100%;'>", "<div class='itemLine'>", "<div class='itemTitle'>", LS.memberDialogFwdAcct, "</div>", "<div class='itemCtrl'><input type='text' id='macct' name='macct' class='acctInput' value='", Fai.encodeHtml(a), "'/></div>", "</div>", "<div class='itemLine'>", "<div class='itemTitle'></div>", "<div class='itemCtrl'><a hidefocus='true' class='formBtn' href='javascript:;' onclick='Site.memberFdPwdStepTwo(", d, ");return false;'>", LS.memberDialogNextStep, "</a></div>", "</div>", "</div>"];
    e = e.join("");
    var b = {
        boxId: d,
...
Site.memberFdPwdStepTwo = function(b) {
    ...
    $.ajax({
        ...
        success: function(c) {
            ...
            if (c.success) {
                if (c.noMail) {
                    var f = ["<div>", "<div style='padding:8px 26px;text-align:center;height:60px;'>", LS.memberDialogNoEmailMsg, "</div>", "<a href='javascript:void(0);' class='formBtn popupBClose' style='margin-left:128px;'>", LS.confirm, "</a>", "</div>"];
                    f = f.join("");
                    var d = {
                        htmlContent: f,
                        width: 378,
    ...
    $.ajax({
        ...
        success: function(i) {
            ...
            if (i.success) {
                ...
                var k = ["<div style='padding-left:60px;'>", "<div class='alertWarn memberFdPwdLastMsg'>", LS.memberDialogReSetPwdSucess, "</div>", "<div style='padding:10px 0 10px 30px;'>", LS.memberDialogFwdAcct, i.macct, "</div>", "<a href='javascript:void(0);' class='formBtn popupBClose' style='margin-left:30px;'>", LS.login, "</a>", "</div>"];
                k = k.join("");
                var j = {
                    htmlContent: k,
                    width: 377,
...
Site.msgBoardAddMsg = function(k) {
    ...
    $.ajax({
        ...
        data: "cmd=add&msgBdData=" + Fai.encodeUrl($.toJSON(i)) + "&validateCode=" + Fai.encodeUrl(j),
        error: function() {
...
Site.msgSubmitAddMsg = function(l, b) {
    ...
    $.ajax({
        type: "post",
        url: "ajax/msgBoard_h.jsp",
        data: "cmd=add&msgBdData=" + Fai.encodeUrl($.toJSON(j)) + "&validateCode=" + Fai.encodeUrl(k) + "&vCodeId=" + Fai.encodeUrl(b),
        error: function() {
...
Site.getDialogHtml = function(r, g) {
    ...
    var k = "";
    var e = "about:blant;";
    var p = LS.goToMallCartStr;
    var a = LS.continueShopping;
    var b = LS.resultFailMsg;
    var n = "resultFailIcon";
    if (!g) {
        g = "javascript:;"
    }
    var i = "mcart.jsp";
    ...
    case Site.MallAjaxErrno.error:
        top.location.href = "mcart.jsp?msg=2";
        return false;
        break;
    ...
    case Site.MallAjaxErrno.login:
        i = "login.jsp?url=" + Fai.encodeUrl(Fai.getUrlRoot(top.location.href)) + "&errno=11";
        top.location.href = i;
        return false;
        break;
    case Site.MallAjaxErrno.idNotExist:
        top.location.href = "mcart.jsp?msg=1";
        return false;
        break;
    case Site.MallAjaxErrno.orderSettle:
        top.location.href = "mcart.jsp";
        return false;
        break;
    ...
    case Site.MallAjaxErrno.toProductDetail:
        top.location.href = "pd.jsp?id=" + productId + "&ram=" + Math.random();
        return false;
        break;
    ...
    var l = [];
    if (r.success) {
        l = ["<div class='mallCartOperate'>", "<a id='settleAccountsBtn' class='formBtn' " + q + " href='" + i + "'>" + p + "</a>", "<a id='dialogContinueShopping' href='javascript:;' class='shopping popupBClose'>", a, "</a>", "</div>"]
    }
    var c = "";
    if (top._lcid === 1033) {
        c = "letter-spacing:0px;"
    }
    var o = ["<table style='width:100%;height:100%;margin-top:-15px;'>", "<tr>", "<td style='width:50px;'></td>", "<td style='width:280px;'>", "<div class='" + n + " addItemTextTips' >" + b + "</div>", "<div class='cartInfoContent' style='" + c + "' >" + f + "</div>", l.join(""), "</td>", "<td style='width:50px;'></td>", "</tr>", "</table>"];
    return o.join("")
};
Site.mallBuying = {};
Site.mallBuy2 = function(d, a, i) {
    if (_siteDemo) {
        Fai.ing("当前为“样板网站”，请先“复制网站”再进行购买。");
        return
    }
    if (typeof(Site.mallBuying[d]) == "undefined") {
        Site.mallBuying[d] = false
    }
    var e = false;
    $.each(Site.mallBuying,
    function(m, l) {
        if (l) {
            e = true;
            return false
    ...
    Site.mallBuying[d] = true;
    var h = "id=" + d;
    if (a && a != "undefined") {
        var c = $("#module" + a);
        var j = c.find(".optionMsg");
        if (Fai.isNull(c)) {
            Site.mallBuying[d] = false;
            return
        }
        ...
        h += "&fromDetail=true";
        c.find(".optionItemWrap").each(function() {
            var m = $(this).parent().parent().find(".propName").text();
            if ($(this).find(".optionItemHover").length == 1) {
                var n = $(this).attr("data");
                var l = $(this).find(".optionItemHover").attr("data");
                var o = {};
                o.name = n;
                o.value = parseInt(l);
                f.push(o);
                ...
                j.show();
                j.html(Fai.format(LS.mallCartChoiceItemError, Fai.encodeHtml(m)));
                k = false;
                return false
            }
        });
        ...
        if (f.length > 0) {
            h += "&optionList=" + Fai.encodeUrl($.toJSON(f))
        }
    }
...
Site.desMallCartAmount = function(c, d, f, k) {
    var e = Fai.top.$("#module" + c);
    var m = e.find(".itemLine" + d);
    var n = m.find(".amountEdit");
    var j = parseInt(n.val());
    ...
    n.val(j);
    if (h != j && i) {
        Site.mallCartAmountChange(c, d, f, k)
    } else {
        var b = parseFloat(m.find(".amountEdit").attr("_itemprice"), 2);
        var l = parseInt(m.find(".amountEdit").val());
        var g = (b * l);
        g = g.toFixed(2);
        m.find(".itemTotalText").text(g)
    }
};
Site.incMallCartAmount = function(c, d, f, k) {
    var e = Fai.top.$("#module" + c);
    var m = e.find(".itemLine" + d);
    var n = m.find(".amountEdit");
    var j = parseInt(n.val());
...
Site.mallCartAmountChange = function(a, b, d, j) {
    var c = Fai.top.$("#module" + a);
    var k = c.find(".itemLine" + b);
    var e = k.find(".amountEdit");
    var g = e.val();
    var f = 1;
    var h = k.find(".J_mcart-pdSelect").prop("checked");
    if (Fai.isInteger(g)) {
        f = parseInt(g)
    }
    if (f < 2) {
    ...
    e.val(f);
    var i = c.find(".cartMsg");
    if (isNaN(f) || f <= 0 || f > 9999999) {
        i.html(LS.mallCartAmountError);
        return
    }
    c.find(".amountEdit").attr("disabled", true);
    i.show();
    i.html(LS.mallCartUpdating);
    $.ajax({
        type: "post",
        url: "ajax/order_h.jsp?cmd=setItem&orderId=" + d,
        data: "itemId=" + j + "&amount=" + f,
        error: function() {
            c.find(".amountEdit").removeAttr("disabled");
            i.html(LS.mallCartUpdateError)
        },
        success: function(l) {
            c.find(".amountEdit").removeAttr("disabled");
            var l = jQuery.parseJSON(l);
            if (l.success) {
                i.html(LS.mallCartUpdateOk);
                var r = 0;
                c.find(".itemLine").each(function() {
                    var v = $(this).find(".J_mcart-pdSelect").prop("checked");
                    var u = parseFloat($(this).find(".amountEdit").attr("_itemprice"), 2);
                    var s = parseInt($(this).find(".amountEdit").val());
                    var t = (u * s);
                    t = t.toFixed(2);
                    $(this).find(".itemTotalText").text(t);
                    if (v) {
                        r += parseFloat(t, 2)
                    }
                });
                var n = c.find(".cartTotalValue");
                var q = n.attr("choiceCurrencyVal");
                var p = q;
                var o = n.attr("levelDiscount");
                var m = n.attr("levelName");
                if (o > 0 && m.length > 0 && o < 100) {
                    p = p + r.toFixed(2) + "*" + o + "%(" + m + ")=" + q;
                    r = r * o / 100
                }
                Site.refreshTopBarMallCartNum();
                n.html(p + r.toFixed(2))
            } else {
                if (l.rt == -3) {
                    i.html(LS.mallCartUpdateNotFound)
                } else {
                    if (l.rt == -9) {
                        i.html(LS.mallCartUpdateStatusError)
                    } else {
                        i.html(LS.mallCartUpdateError)
                    }
                }
...
Site.mallCartItemDel = function(g, d, a, f, c) {
    var b = Fai.top.$("#module" + g);
    var e = b.find(".cartMsg");
    e.show();
    e.html(LS.mallCartUpdating);
    $.ajax({
        type: "post",
        url: "ajax/order_h.jsp?cmd=delItem&orderId=" + a,
        data: "itemId=" + f,
        error: function() {
            e.html(LS.mallCartUpdateError)
        },
        success: function(p) {
            var p = jQuery.parseJSON(p);
            if (p.success) {
                e.html(LS.mallCartUpdateOk);
                b.find(".itemLine" + d).remove();
                b.find(".separatorLine" + d).remove();
                var o = b.find(".itemList_" + c).find(".itemLine");
                if (o.length == 0) {
                    b.find(".itemList_" + c).remove()
                }
                var n = b.find(".itemLine");
                if (n.length == 0) {
                    Fai.top.location.reload()
                } else {
                    var m = 0;
                    n.each(function() {
                        var s = parseFloat($(this).find(".amountEdit").attr("_itemprice"), 2);
                        var q = parseInt($(this).find(".amountEdit").val());
                        var r = (s * q);
                        r = r.toFixed(2);
                        $(this).find(".itemTotalText").text(r);
                        m += parseFloat(r, 2)
                    });

                    var i = b.find(".cartTotalValue");
                    var k = i.attr("choiceCurrencyVal");
                    var h = i.attr("levelDiscount");
                    var l = i.attr("levelName");
                    var j = k;
                    if (h > 0 && h < 100) {
                        j += m.toFixed(2) + "*" + h + "%(" + l + ")=" + k;
                        m = m * h / 100
                    }
                    i.html(j + m.toFixed(2));
                    Site.refreshTopBarMallCartNum()
                }
            } else {
                if (p.rt == -3) {
                    e.html(LS.mallCartUpdateNotFound)
                } else {
                    if (p.rt == -9) {
                        e.html(LS.mallCartUpdateStatusError)
                    } else {
                        e.html(LS.mallCartUpdateError)
                    }
                }
...
Site.mallSettle = function(a) {
    var b = $(".J_mcart-pdSelect").not("input:checked");
    var c = new Array();
    $(b).each(function() {
        var d = $(this).attr("itemId");
        c.push(d)
    });
    if (a > 0) {
        Fai.top.location.href = "mstl.jsp?id=" + a + "&unCheckedItemIds=" + c
    } else {
...
Site.mallStlCalcTotal = function(h) {
    ...
    if ($("#offsetMoney").length > 0) {
        ...
        if (!isNaN(c)) {
            g = parseFloat(c);
            if ((f + d) < g) {
                Fai.ing(Fai.format(LS.integral_maxUse, $("#useItg").attr("_maxuse"), Fai.encodeHtml($("#useItg").attr("__itegName"))));
                $("#useItg").focus();
...
Site.mallSubmit = function(d, q, i, g) {
    if (q == 0) {
        Fai.top.location.href = "mdetail.jsp";
        return
    }
    var u = {};
    var d = Fai.top.$("#module" + d);
    var l = d.find(".stlMsg");
    var j = "";
    d.find(".propItemValue").each(function() {
        var w = $(this).find("input").val();
        if ($(this).attr("_required") == 1 && !w) {
            j = LS.mallStlSubmitInput + $(this).attr("_prop");
            return false
        }
        var v = $(this).attr("_field");
        if (w && w.length > 0) {
            if (v === "email" && !Fai.isEmail(w)) {
                j = LS.mallStlSubmitInput2 + $(this).attr("_prop");
                return false
            }
            if (v === "phone" && !Fai.isPhone(w)) {
                j = LS.mallStlSubmitInput2 + $(this).attr("_prop");
                return false
            }
        }
    ...
    var o = d.find("input[name=mallPay]:checked");
    if (o.length == 1) {
        u.payMode = parseInt(o.val())
    }
    var p = parseInt(o.val());
    if (p == 3) {
        var r = d.find("input[name=onlineTenpayBankType]:checked")
    } else {
        var r = d.find("input[name=onlineAlipayBankType]:checked")
    }
    if (r.length == 1) {
        if (p != 6 && p != 7 && p != 9) {
            u.payBankType = parseInt(r.val())
        }
    }
    var t = $("#useItg");
    if (t.length > 0) {
        var c = t.val();
        if (Fai.isInteger(c)) {
            var e = parseInt(t.attr("_maxUse"));
            var a = parseInt(t.attr("_currentitg"));
            var s = t.attr("_itgname");
            if (c > a) {
                Fai.ing(Fai.format(LS.integral_notOverCurrent, Fai.encodeHtml(s), Fai.encodeHtml(s)));
                t.focus();
                return
            }
            if (c > e) {
                Fai.ing(Fai.format(LS.integral_notOver, Fai.encodeHtml(s), e));
                t.focus();
                return
            }
            if (c < 0) {
    ...
    var m = d.find(".mallStlOpt input");
    m.attr("disabled", true);
    l.show();
    l.html(LS.mallStlSubmitting);
    Site.scrollToDiv(d);
    $.ajax({
        type: "post",
        ...
        error: function() {
            ...
            l.html(LS.mallStlSubmitError)
        },
        success: function() {
            $.ajax({
                type: "post",
                url: "ajax/order_h.jsp?cmd=settle&orderId=" + q,
                data: "data=" + Fai.encodeUrl($.toJSON(u)),
                error: function() {
                    m.removeAttr("disabled");
                    l.html(LS.mallStlSubmitError)
                },
                success: function(v) {
                    m.removeAttr("disabled");
                    var v = jQuery.parseJSON(v);
                    if (v.success) {
                        $.ajax({
                            type: "post",
                            url: "ajax/order_h.jsp?cmd=addAfterShop",
                            data: "data=" + Fai.encodeUrl($.toJSON(g)),
                            error: function() {
                                m.removeAttr("disabled");
                                l.html(LS.mallStlSubmitError)
                            },
                            success: function(w) {
                                Fai.top.location.href = "mdetail.jsp?id=" + q + "&suc=true"
                            }
                        })
                    } else {
                        if (v.rt == -3) {
                            l.html(LS.mallStlSubmitNotFound)
                        } else {
                            if (v.rt == -9) {
                                l.html(LS.mallStlSubmitStatusError)
                            } else {
                                if (v.rt == Site.MallAjaxErrno.outOfMallAmount) {
                                    l.html(Fai.format(LS.mallAmountOverNameList, v.productsName))
                                } else {
                                    l.html(LS.mallStlSubmitError)
                                }
                            }
...
Site.initModuleMallShipTemplate = function(n, o, a, q, f, b) {
    var d = q.vType || 1,
    j = q.sco || 0,
    c = q.openItemList;
    var i = "-----------",
    ...
    r,
    g,
    l = site_cityUtil.getProvince();
    site_cityUtil.simpleProvinceName(l);
    $.each(l,
    function(s, t) {
        m.push("<option value='" + t.id + "' >" + site_cityUtil.simpleCityNameStr(t.name) + "</option>")
    });
    $("#mallShipTemplate_province").html("").html("<option value='-1'>" + i + "</option>" + m.join("")).change(function(s) {
        e = $("#mallShipTemplate_province").val();
        if (isNaN(e) || e <= 0) {
            $("#mallShipTemplate_city").html("").html("<option value='-1'>" + i + "</option>");
            $("#mallShipTemplate_county").html("").html("<option value='-1'>" + i + "</option>");
            $.each(c,
            function(t, u) {
                $("#template_item_maoney" + u.type).html("").html(f + u.defaultPrice.toFixed(1));
        ...
        h = [];
        p = site_cityUtil.getCities(e);
        site_cityUtil.simpleCityName(p);
        $.each(p,
        function(t, u) {
            h.push("<option value='" + u.id + "' >" + u.name + "</option>")
        });
        $("#mallShipTemplate_city").html("").html("<option value='-1'>" + i + "</option>" + h.join("")).unbind().bind("change",
        function(t) {
            k = $("#mallShipTemplate_city").val();
            if (isNaN(k) || k <= 0) {
                $("#mallShipTemplate_county").html("").html("<option value='-1'>" + i + "</option>");
                $.each(c,
                function(u, v) {
                    $("#template_item_maoney" + v.type).html("").html(f + v.defaultPrice.toFixed(1));
            ...
            g = [];
            r = site_cityUtil.getCounty(k);
            $.each(r,
            function(u, v) {
                g.push("<option value='" + v.id + "' >" + v.name + "</option>")
            });
            $("#mallShipTemplate_county").html("").html("<option value='-1'>" + i + "</option>" + g.join("")).unbind().bind("change",
            function(u) {
                Site.mallStlCalcTotal(n)
            });
            if (o === 0) {
            ...
            function(v, x) {
                if (!x.regionList || x.regionList.length <= 0) {
                    $("#template_item_maoney" + x.type).html("").html((f + x.defaultPrice.toFixed(1)));
                ...
                if (j) {
                    ...
                    if (w) {
                        $("#template_item_maoney" + x.type).html("").html((f + 0));
                ...
                $.each(x.regionList,
                function(D, G) {
                    if ($.inArray(parseInt(k), G.areaList) < 0) {
                        $("#template_item_maoney" + x.type).html("").html((f + x.defaultPrice.toFixed(1)));
                        $("#template_item_" + x.type).attr("price", x.defaultPrice.toFixed(1));
                        return true
                    }
                    var C = 0,
                    z = G.price,
                    E = G.rha == null ? 1 : G.rha,
                    F = G.ria == null ? 1 : G.ria,
                    A = G.rip || 0;
                    var B = o;
                    if (E > 0) {
                        C = z;
                    ...
                    $("#template_item_maoney" + x.type).html("").html((f + C.toFixed(1)));
                    $("#template_item_" + x.type).attr("price", C.toFixed(1));
                    return false
                })
            });
            Site.mallStlCalcTotal(n)
    ...
    $("#mallShipTemplate_county").html("").html("<option value='-1'>" + i + "</option>");
    setTimeout(function() {
        $("#mallShipTemplate_province").val(b.provinceCode);
        $("#mallShipTemplate_province").trigger("change");
        setTimeout(function() {
            $("#mallShipTemplate_city").val(b.cityCode);
            $("#mallShipTemplate_city").trigger("change");
            setTimeout(function() {
                $("#mallShipTemplate_county").val(b.countyCode);
    ...
    0);
    $("#template_item_" + b.type).click();
    $("#streetAddress").val(b.streetAddr)
};
Site.initPresentIpt = function() {
    ...
    if (a.length > 0) {
        ...
        function() {
            ...
            if (g > c) {
                Fai.ing(Fai.format(LS.integral_maxUse, $("#useItg").attr("_maxuse"), Fai.encodeHtml($("#useItg").attr("_itgName"))));
                return
            }
...
Site.mallCartInit = function(b) {
    ...
    $(a).find(".mallCartItem").bind("click",
    function() {
        window.open("mcart.jsp", "_blank")
    });
    $(c).find(".checkMallCartBtn").bind("click",
    function() {
        window.open("mcart.jsp", "_blank")
    });
    Site.refreshTopBarMallCartNum();
...
Site.refreshTopBarMallCart = function(a) {
    ...
    $.ajax({
        type: "post",
        url: "ajax/order_h.jsp?cmd=getTopBarMallCartList",
        data: "",
        success: function(g) {
            var h = $.parseJSON(g);
...
Site.createTopBarMallCartList = function(b, a, l) {
    ...
    m = (m > 5) ? (m - 5) : 0;
    k.push("<ul>");
    k.push("<div style='text-align: left;'>" + LS.memberMallCartJoin + "：</div>");
    for (var j in b) {
        var c = b[j];
        var f = c.amount;
        if (f > 1) {
    ...
    if (m > 0) {
        var d = "<span>" + m + "</span>";
        k.push("<li class='mcProductListTip'>" + Fai.format(LS.memberMallCartProductNum, d) + "</li>");
        Fai.top.$("#memberBar").find(".checkMallCartBtn").css("margin-top", "0px")
    } else {
        k.push("<li class='mcProductListTip'></li>");
        Fai.top.$("#memberBar").find(".checkMallCartBtn").css("margin-top", "12px")
    }
    k.push("</ul>");
    Fai.top.$("#memberBar").find(".mcProductList").append(k.join(""));
    Fai.top.$("#memberBar").find(".checkMallCartBtn").addClass("checkMallCartBtn_hasPro");
    function g(n) {
        var i = [];
        if (c.picId != "") {
            i.push("<div class='mcProductList_proPic'>");
            i.push("<img alt=" + Fai.encodeHtml(c.name) + " src=" + c.picPath + ">");
            i.push("</div>")
        } else {
            i.push("<div class='mcProductList_proNoPic'></div>")
        }
        k.push("<li class='mcProductList_pro' data-itemId=" + c.id + ">");
        k.push(i.join(""));
        k.push("<div class='mcProductList_proName'>");
        k.push("<span>" + Fai.encodeHtml(c.name) + "</span>");
        k.push("</div>");
        k.push("<div class='mcProductList_proPrice'>");
        k.push("<span>" + l.toString() + "</span><span style='color: #ff4400; font-weight: bold;'>" + c.price + "</span>");
        k.push("</div>");
        k.push("<div class='mcProductList_proDel'><a hidefocus='true' href='javascript:;' onclick='Site.delTopBarMallCartItem(" + a + ", " + c.id + ");return false;'>" + LS.memberMallCartDel + "</a></div>");
        k.push("</li>")
    }
};
Site.delTopBarMallCartItem = function(a, b) {
    $.ajax({
        type: "post",
        url: "ajax/order_h.jsp?cmd=delItem",
        data: {
            orderId: a,
            itemId: b,
            isTopBarMallCart: true
        },
        error: function(c) {
            var d = $.parseJSON(c);
            if (!d.success) {
...
Site.richMarquee = function(c) {
    ...
    f = "top",
    n = r.speed,
    t = (r.loop != "infinite" || r.loop <= 0) ? 1 : r.loop,
    j = r.moveout,
    w = r.isScrolling,
    u = 20,
    q = $("<div id='richMarqueeCopy" + h + "' class='richMarquee' style='position:absolute; overflow:hidden; width:" + g + "px; height:" + p.height() + "px ' ></div>"),
    k = $("<div id='richmarqueeNewParent" + h + "' style='overflow:hidden;width:" + g * 2 + "'></div>");
    function m() {
...
Site.restartRichMarquee = function(a) {
    ...
    if (!b.data("first2")) {
        ...
        b.on("Fai_onModuleSizeChange",
        function() {
            Fai.stopInterval("rm" + a.id);
            b.data("restart", true);
            Site.richMarquee(a)
        });
        b.on("Fai_onModuleLayoutChange",
        function() {
            Fai.stopInterval("rm" + a.id);
            b.data("restart", true);
            Site.richMarquee(a)
...
Site.loadProductMarquee = function(v, H, C, t) {
    var f = Fai.top["Product" + v].ieOpt,
    h = Fai.top["Product" + v].tgOpt,
    o = Fai.top["Product" + v].callbackArgs;
    ...
    if (t == 0 || t == 1) {
        ...
        z = A.width();
        if (z <= a) {
            Site.imageEffect(v, f, h, Site.createImageEffectContent_product, o);
            return
        }
    }
    if (t == 2 || t == 3) {
    ...
    s.each(function(J, L) {
        ...
        var I = $(L).clone().attr("cloneDiv", "true"),
        M = I.children("div.imgDiv").attr("id");
        ...
        if (Fai.isIE6() || Fai.isIE7()) {
            if (J === d) {
                $(s[J]).after('<div style="clear:both;"></div>');
...
Site.loadProductTextList = function(b) {
    ...
    a.find(".productTextListTable").each(function(h, l) {
        var j = $(l).find("tr");
        for (var h = 0; h < j.length; ++h) {
            ...
            var r = 100;
            var d = f;
            Number(p.split("p")[0]) / Number(q.split("p")[0]);
            if (g.css("background-image").lastIndexOf("url") == -1) {} else {
                r -= 1
            }
            if (f === 2) {
                $(g[1]).css("width", r + "%");
    ...
    a.find(".productTextListProp").each(function() {
        var h = $(this).find("tr");
        for (var g = 0; g < h.length; ++g) {
            ...
            var m = 100;
            var d = e;
            if (f.css("background-image").lastIndexOf("url") == -1) {} else {
                m -= 1
            }
            if (e === 2) {
                $(f[1]).css("width", m + "%");
    ...
    if (!_manageMode) {
        a.find(".productTextListTable").bind("mouseenter",
        function() {
            $(this).addClass("g_hover")
        }).bind("mouseleave",
        function() {
...
Site.initProductOptions = function(f, b, c, a) {
    var d = $("#module" + f);
    if (Fai.isNull(d)) {
        return
    }
    ...
    d.find(".optionItemWrap .optionItem a").hover(function() {
    ...
    d.find(".optionItemWrap").each(function() {
        $(this).find(".optionItem").bind("click",
        function() {
            $(this).find("a").addClass("optionItemHover g_borderSelected").end().siblings().find("a").removeClass("optionItemHover g_borderSelected").addClass("g_border");
            var k = $(".optionItemWrap").length;
            var i = $(".optionItemWrap .g_borderSelected");
            if (k == i.length) {
                var h = "";
                var j;
                var g;
                ...
                if (j == null) {
                    $("#realMallAmount").text(a)
                } else {
                    $("#realMallAmount").text(j);
                    if (Fai.isIE10()) {
                        if (Site.wipeOffRunOne) {
                            $(".pd_J .pd_propTable .propName").each(function(l, m) {
                                $(m).removeClass("propName");
                                setTimeout(function() {
                                    $(m).addClass("propName")
                                },
                                0)
...
Site.loadProductDetail = function(l, o, i, g) {
    ...
    if (t.length > 1) {
        ...
        Site.multiPhoto($("#imgDivs" + l), $("#imgDiv" + l), {
            leftIconID: "leftIcon" + l,
            rightIconID: "rightIcon" + l,
            zoom: u
        })
    }
    var d = c.find(".pd_J").width(),
    e = c.find(".imgContainer_J").width(),
    r = c.find(".pdLayoutR_J"),
    k = c.find(".pdAppendLayout_J");
    if ((d - e) < 200) {
        r.appendTo(k)
    }
};
Site.initPdCommentSwitch = function() {
    ...
    if (document.cookie.length > 0) {
        cStart = document.cookie.indexOf("openProductCom=");
        if ( - 1 != cStart) {
            cStart = cStart + 15;
            ...
            cookieV = unescape(document.cookie.substring(cStart, cEnd));
            if ("true" == cookieV) {
                a = true
            }
        }
    }
    if ((a && $("#accComSwitch").length > 0) || ($("#detDescSwitch").length <= 0 && $("#accComSwitch").length > 0)) {
        $("#accComSwitch").click()
    } else {
        if ($("#detDescSwitch").length > 0) {
            $("#detDescSwitch").click()
        }
    }
};
...
Site.loadProductGallery = function(w) {
    ...
    if (v.length != 0) {
        ...
        var L = b.find("a.gallery-control"),
        o = b.find("a.gallery-control-prev"),
        Q = b.find("a.gallery-control-next"),
        J = o.height(),
        c = o.width();
        var g = v.first(),
        R = g.children("div.img-container"),
        C = R.outerWidth(true),
        s = R.outerHeight(true),
        A = g.find("div.prop-container"),
        f = 0;
        if (A.length > 0) {
        ...
        z.height(r);
        var y = b.find("div.product-gallery-inner"),
        n = y.outerHeight();
        if (n < J) {
            var M = y.height(),
            P = n - M;
        ...
        o.click(function() {
            ...
            var Z = x.parent(),
            j = Z.find("div.product-gallery-container"),
            aa = Z.find("a.gallery-control-next");
            if (aa.hasClass("gallery-control-next-disabled")) {
                aa.removeClass("gallery-control-next-disabled")
            }
        ...
        Q.click(function() {
            ...
            var Z = ae.parent(),
            i = Z.find("div.product-gallery-container"),
            x = Z.find("a.gallery-control-prev");
            if (x.hasClass("gallery-control-prev-disabled")) {
                x.removeClass("gallery-control-prev-disabled")
            }
    ...
    if (b.find(".prop-wordwrap-container").length > 0) {
        ...
        var y = b.find("div.product-gallery-inner"),
        L = b.find("a.gallery-control"),
        o = b.find("a.gallery-control-prev"),
        J = o.height(),
        I = (y.outerHeight() / 2) - (J / 2);
        L.css("top", I + "px")
    }
};
Site.loadProductSmallPic = function(i) {
    ...
    E = i.bigPicObjs;
    var e = $("#" + r);
    e.on("Fai_onModuleSizeChange",
    function() {
        Site.smallPicModuleFix(r)
    });
    e.on("Fai_onModuleLayoutChange",
    function() {
        Site.smallPicModuleFix(r)
    });
    ...
    var u = e.find(".smallPicDownForms a.g_imgPrev"),
    v = e.find(".smallPicDownForms a.g_imgNext"),
    l = e.find(".smallPicUpForms a.g_control_prev"),
    m = e.find(".smallPicUpForms a.g_control_next");
    ...
    v.click(function() {
        ...
        var j = S.parents(".smallPicDownForms").find(".smallPrePicContainer"),
        L = S.parents(".smallPicDownForms").find("a.g_imgPrev"),
        K = Math.floor(J.find(".smallPicDownFormsMid").width() / a) + 1;
        if (L.hasClass("smallPic-control-prev-disabled")) {
            L.removeClass("smallPic-control-prev-disabled")
        }
        var T = (K - 1) * a,
        R = parseInt(j.css("left").replace("px"));
        var Q = Math.abs(R) / a;
    ...
    g.click(function() {
        ...
        var Q = E[ai]["bWidth"];
        var T = E[ai]["bHeight"];
        var j = L.find(".containerLeft").width(),
        I = L.find(".containerLeft").height();
        if (ae) {
        ...
        var Y = [];
        Y.push("<div class='loadingImg' style='width:" + j + "px;height:" + I + "px;'>");
        Y.push("<table cellspacing='0' cellpadding='0' class='loadingImgTable' style='width:" + j + "px;height:" + I + "px;'><tbody><tr><td class='loadingImgTd'>");
        Y.push("<div class='ajaxLoading2' style='margin:0 auto;'></div>");
        ...
        var ak = "word-break:normal;white-space:normal;overflow:visible;text-overflow:clip";
        if (ad == "false") {
            ak = "white-space:nowrap;overflow:hidden;text-overflow:ellipsis"
        }
        var Y = [];
        var X = L.find(".containerRight").width();
        Y.push("<div class='productParamContainer' style='width:" + X + "px;" + J + "'>");
        if (ao == "true") {
            Y.push("<a " + am + " title='" + $(this).find("img").attr("productname") + "' style='text-decoration:none;'><div style='width:" + (X - 20) + "px;margin:auto;" + N + ";margin-top:20px;margin-bottom:8px;" + ak + ";'>" + Fai.encodeHtml($(this).find("img").attr("productname")) + "</div></a>")
        }
        var P = D[ai];
        for (var Z in P) {
            if (typeof(P[Z]) != "function") {
                ...
                Y.push("<div style='margin:auto;height:20px;line-height:20px;padding: 5px 0;width:" + (X - 20) + "px;" + N + ";text-overflow:ellipsis;white-space:nowrap;overflow:hidden;'>");
                if (i.propNameShow) {
                    Y.push("<span>" + Z + "：</span>")
                }
        ...
        if (M == "true") {
            Y.push("<div style='margin-left:9px;height:20px;line-height:20px;padding: 5px 0;width:" + (X - 20) + "px;" + N + ";text-overflow:ellipsis;white-space:nowrap;overflow:hidden;'>");
            Y.push("<a hidefocus='true' class='g_mallBuy' href='javascript:;' onclick='" + O + "'><span>");
            Y.push(ab);
            Y.push("</span></a>");
            Y.push("</div>")
        }
...
Site.smallPicModuleFix = function(w) {
    ...
    var q = e.find(".smallPicDownForms a.g_imgPrev"),
    r = e.find(".smallPicDownForms a.g_imgNext"),
    i = e.find(".smallPicUpForms a.g_control_prev"),
    k = e.find(".smallPicUpForms a.g_control_next");
    if (v == 1) {
...
Site.productCommentSwitchClick = function(a) {
    if (true == a) {
        $(".selected").removeClass("g_borderSelected");
        $(".selected div").removeClass("pdNoBottomBorder");
        $("#detDescSwitch").removeClass("selected");
        $("#detDescSwitch").addClass("g_border");
        $("#accComSwitch").addClass("selected");
        $(".selected").addClass("g_borderSelected");
        $(".selected div").addClass("pdNoBottomBorder");
        $("#accComSwitch").removeClass("g_border");
        $("#detailedDesc").hide();
        $("#accumulativeComment").show();
        document.cookie = "openProductCom=true"
    } else {
        $(".selected").removeClass("g_borderSelected");
        $(".selected div").removeClass("pdNoBottomBorder");
        $("#accComSwitch").removeClass("selected");
        $("#accComSwitch").addClass("g_border");
        $("#detDescSwitch").addClass("selected");
        $(".selected").addClass("g_borderSelected");
        $(".selected div").addClass("pdNoBottomBorder");
        $("#detDescSwitch").removeClass("g_border");
        $("#accumulativeComment").hide();
        $("#detailedDesc").show();
        document.cookie = "openProductCom=false"
    }
...
Site.productCommentAddCom = function() {
    ...
    var a = $.trim($("#productCommentCreator").val());
    var g = $("#productCommentCreator").attr("minlength");
    var f = $("#productCommentInput").val();
    var h = $("#productCommentInput").attr("minlength");
    var i = $("#validateCodeInput").val();
    var c = $("#productCommentInput").attr("productId");
    var b = $("#submitTips").attr("isDefaultTips");
    if (typeof(b) == "string" && b == "true") {
        $("#submitTips").show();
        return
    }
    if (typeof(a) != "string") {
    ...
    var e = "ajax/productComment_h.jsp?cmd=submitPC";
...
Site.initModuleProductMallGroups = function(g, f, c, b) {
    var e = $("#module" + g),
    d = e.find(".pd_mall_G_J .p_m_cotainer_J");
    $(d).hover(function() {
        var F = $(this);
        F.find(".p_m_cotainerR_J").removeClass("p_m_more");
        F.find(".p_m_value_J").addClass("g_stress");
        F.addClass("bold p_m_hover g_border");
        var x = F.attr("_id"),
        q = f[x];
        if (q == null || q.list.length === 0) {
            return true
        }
        var C = a(q);
        var n = {
            that: F,
            moduleId: g,
            clsStr: "pd_m_panel " + (c === 1 || c === 2 ? "pd_m_jd": "pd_m_yhd"),
            idStr: "group" + x + "Panel",
            contentStr: C.join("")
        };
        panel = Site.moduleSubPanel(n);
        var i = e.find(".formBanner"),
        I = 0;
        if (i.length > 0 && i.is(":visible")) {
            I = i.outerHeight()
        }
        var u = e.find(".formMiddleRight").outerWidth();
        if (Fai.isIE8()) {
            u = 0
        }
        var k = e.find(".formMiddleCenter").width(),
        o = e.find(".pd_mall_G_J").width(),
        H = e.offset().top + I,
        m = e.offset().left + e.outerWidth() - ((k - o) / 2) - 2 - u;
        panel.css({
            top: H,
            left: m
        }).show();
        if (Fai.isIE6()) {
            var J = panel.find(".p_m_body_J").outerWidth();
            panel.width(J)
        }
        var t = 0,
        h = 0,
        D = F.outerHeight() - F.innerHeight();
        m_border = parseInt((e.outerHeight() - e.innerHeight()) / 2);
        oFormMiddle = e.find(".formMiddle"),
        ...
        if (c === 2) {
            ...
            l = panel.outerHeight(),
            w = $(window).height(),
            A = F.offset().top - $(window).scrollTop(),
            r = w - A,
            B = 0;
            if (r < l) {
                B = l - r + s
            ...
            var v = oFormMiddle.height(),
            E = parseInt((oFormMiddle.outerHeight() - oFormMiddle.innerHeight()) / 2);
            H += F.position().top - B + m_border + E + m_content_innerPaddingTop;
            panel.css({
                top: H
            });
            h = B + D / 2;
            t = y > l ? l - 4 : F.outerHeight() - D
        } else {
            var G = e.find(".formMiddleContent").offset().top - e.find(".formMiddleCenter").offset().top,
            z = F.position().top + F.outerHeight() + G,
            j = panel.outerHeight();
            if (j - 5 < z) {
                var p = panel.find(".p_m_body_J");
                p.height(z)
            }
            h = F.position().top + D / 2 + m_border + m_content_innerPaddingTop;
            t = F.outerHeight() - D
        }
        ...
        panel.mouseleave(function() {
            F.removeClass("bold p_m_hover g_border");
            F.find(".p_m_value_J").removeClass("g_stress");
            if (F.attr("_chd") == 1) {
                F.find(".p_m_cotainerR_J").addClass("p_m_more")
            }
        })
    },
    function(i) {
        var h = $(this);
        h.removeClass("bold p_m_hover g_border");
        h.find(".p_m_value_J").removeClass("g_stress");
        if (h.attr("_chd") == 1) {
            h.find(".p_m_cotainerR_J").addClass("p_m_more")
        }
    });
    function a(h) {
        var j = [];
        j.push("<div class='p_m_cover p_m_cover_J'></div>");
        j.push("<div class='" + (h.hasGrand ? "p_m_body2": "p_m_body") + " g_border p_m_body_J'>");
        var i = "";
        if (b === 1) {
            i = "target='_blank'"
        }
        if (h.hasGrand) {
            if (c === 1 || c === 2) {
                ...
                function(k, l) {
                    if (b !== 1 && _manageMode) {
                        i = "onclick=\"Site.redirectUrl('" + l._href + "', '_self', event, 1, 0);return false;\""
                    }
                    j.push("<dl>");
                    j.push("<dt><a class='g_stress' href='" + l._href + "' " + i + ">" + Fai.encodeHtml(l.name) + "</a></dt>");
                    j.push("<dd>");
                    $.each(l.children,
                    function(m, n) {
                        if (b !== 1 && _manageMode) {
                            i = "onclick=\"Site.redirectUrl('" + n._href + "', '_self', event, 1, 0);return false;\""
                        }
                        j.push("<a href='" + n._href + "' " + i + " title='" + Fai.encodeHtml(n.name) + "'>" + Fai.encodeHtml(n.name) + "</a>")
                    });
                    j.push("</dd>");
                    j.push("</dl>");
                    j.push("<div class='p_m_sep'></div>")
                })
            } else {
                $.each(h.list,
                function(k, l) {
                    ...
                    function(m, n) {
                        ...
                        j.push("<dd><a href='" + n._href + "' " + i + " title='" + Fai.encodeHtml(n.name) + "'>" + Fai.encodeHtml(n.name) + "</a></dd>")
                    });
                    j.push("</dl>");
                    if ((k + 1) % 3 == 0 || (k + 1) == h.list.length) {
                        j.push("<div class='p_m_sep'></div>")
                    }
                })
            }
            ...
            function(k, l) {
                ...
                j.push("<a class='p_m_line g_stress' href='" + l._href + "' " + i + ">" + Fai.encodeHtml(l.name) + "</a>")
            })
        }
        j.push("</div>");
        return j
...
Site.createImageEffectContent_product = function(target, ieParam, tgParam, extData) {
    ...
    bindPropsEvent();
    calPropPadding();
    if (ieParam.effType == 5) {
        $(targetImgEff).css("height", calPropsHeight() + "px").css("bottom", "-" + calPropsHeight() + "px")
    }
    function generateProps() {
        ...
        propListStyle += (tgParam.productTextCenter ? "text-align: center;": "text-align: left;");
        ...
        if (tgParam.productNameShow) {
            $(propsList).append("<a href='javascript:;' onclick='return false;' class='imgEffPropName' style='" + (tgParam.productNameWordWrap ? "word-wrap: break-word; overflow: visible; text-overflow: clip; white-space: normal;": "") + "'>" + productName + "</a>")
        }
        if (props.length > 0) {
    ...
    function generatePropBuy() {
        if (tgParam.mallShowBuy) {
            ...
            $(targetImgEff).find(".props .propBuy").css("display", "none")
        }
    }
    function bindPropsEvent() {
        $(targetImgEff).find(".propBuy").bind("mouseenter",
        function() {
        ...
        if (Fai.isIE6() || Fai.isIE7()) {
            $(targetImgEff).find(".propBuy").bind("click",
            function() {
    ...
    function calPropPadding() {
        if (ieParam.effType == 5) {
            return
        }
        var imgEffHeight = $(targetImgEff).height();
        var propsHeight = $(targetImgEff).find(".props").height();
        var propList = $(targetImgEff).find(".props .propList");
        var propListPadding = (imgEffHeight - propsHeight) / 2;
        $(propList).css("padding-top", propListPadding + "px")
    }
    function calPropsHeight() {
        var propList = $(targetImgEff).find(".props .propList");
        var propBuy = $(targetImgEff).find(".props .propBuy");
        var imgPropListHeight = $(propList).height();
        var propList_MarginTop = parseInt($(propList).css("padding-top"));
        var propList_MarginBottom = parseInt($(propList).css("margin-bottom"));
        var imgEffBuyHeight = 0;
        if ($(propBuy).length > 0) {
            var propBuy_MarginTop = parseInt($(propBuy).css("margin-top"));
            imgEffBuyHeight = $(propBuy).height() + propBuy_MarginTop + 8
        }
        imgPropListHeight += propList_MarginTop + propList_MarginBottom;
        return imgPropListHeight + imgEffBuyHeight
    }
};
...
Site.initModuleProductResultPropFilter = function(d) {
    ...
    if (Fai.top._manageMode) {
        $("#pf_tips").hover(function() {
            ...
            tipImgW = g.width(),
            tipTop = g.offset().top,
            tipLeft = g.offset().left,
            str = "<div id='pf_tips_Msg' class='pf_tips_Msg g_tip'>" + g.attr("_tipVal") + "</div>";
...
Site.loadNewsList = function(d, c) {
    ...
    b.find(".lineBody").each(function() {
        var k = $(this);
        var h = k.find(".newsType");
        if (h.length != 0) {
            var f = h.find(">a").first().outerWidth();
            var e = f + Fai.getCssInt(h, "padding-left") + Fai.getCssInt(h, "padding-right") + 5;
            if (typeof a.nt == "undefined" || a.nt < e) {
                if (e > 300) {
                    e = 300
                }
                a.nt = e
        ...
        if (j.length != 0) {
            var i = j.find(">a").first().outerWidth();
            var g = i + Fai.getCssInt(j, "padding-left") + Fai.getCssInt(j, "padding-right") + 5;
            if (typeof a.nc == "undefined" || a.nc < g) {
                a.nc = g
            }
        }
    });
...
Site.siteFormAdd = function(b, o, m, e, a) {
                ...
                if (z == 1) {
                    ...
                    if (z == 2) {
                        ...
                        if (z == 3) {
                            ...
                            if (z == 4) {
                                if (B && $("#M" + b + "F" + o + "siteFormSelect" + s + " option:selected").length == 0) {
                                    Site.siteFormShowMsg(Fai.format(LS.siteFormSubmitCheckIsEmpty, Fai.encodeHtml(p)), o, b);
                                    c = false;
                                    return false
                                }
    ...
    $.ajax({
        ...
        data: "cmd=addSubmit&formId=" + o + "&submitContentList=" + Fai.encodeUrl($.toJSON(g)) + "&vCodeId=" + b + o + "&validateCode=" + h + "&tmpFileList=" + Fai.encodeUrl($.toJSON(i)),
        error: function() {
        ...
        success: function(p) {
            p = $.parseJSON(p);
            if (p.success) {
                Site.siteFormShowMsg(LS.siteFormSubmitOk, o, b);
...
Site.siteFormFileUpload = function(a, c, f, e, d) {
    var b = {
        file_post_name: "Filedata",
        upload_url: "/ajax/upsiteimg_h.jsp",
        button_placeholder_id: "siteForm" + e + "fileUpload" + c,
        ...
        upload_success_handler: function(h, g) {
            ...
            if (i.success) {
                ...
                onFileUploadEvent("upload", i, e, c)
            } else {
                Fai.ing("文件" + h.name + "，" + i.msg)
            }
        },
        upload_error_handler: function(h, g, i) {
            if (g == -280) {
                ...
                if (g == -270) {
                    Fai.ing("已经存在名称为" + h.name + "的文件。", true)
                } else {
                    Fai.ing("服务繁忙，文件" + h.name + "上传失败，请稍候重试。")
                }
            }
        },
        ...
        view_progress: function(g, j, i, h) {
            Fai.ing("正在上传" + h + "%", false)
        }
    };
    swfObj = SWFUploadCreator.create(b);
    onFileUploadEvent = function(t, q, u, n) {
        if (t == "upload") {
            var k = q.id;
            var s = q.name;
            var i = q.type;
            ...
            var r = 100;
            var m = q.fileId;
            $("#module" + u).find("#siteForm" + u + "fileName" + n).html(s);
            $("#module" + u).find("#siteForm" + u + "fileName" + n).attr("_tmpFileId", k).attr("_tmpFileName", s).attr("title", s).attr("_fileId", m)
        }
    }
};
...
Site.saveNoticeMarqueeProps = function(a) {
    ...
    $("#module" + a.id).on("Fai_onModuleSizeChange",
    function() {
        Site.noticeMarquee($("#noticeMarquee" + a.id).data("options"), a.id)
    });
    $("#module" + a.id).on("Fai_onModulePositionChange",
    function() {
        Site.noticeMarquee($("#noticeMarquee" + a.id).data("options"), a.id)
    })
};
...
Site.bindLocalerItemEvent = function() {
    $("#localer").find(".J_localerPanel .J_localerList .localerItemContent").bind("mouseenter",
    function() {
...
Site.setModuleHeight2 = function(m, l) {
    var c = Fai.top.$("#module" + m);
    c.css("height", l + "px");
    if (c.hasClass("formStyle80")) {
        return
    }
    var h = c.find(".formTop" + m);
    var k = c.find(".formBanner" + m);
    var j = c.find(".formMiddle" + m);
    var p = c.find(".formBottom" + m);
    var b = h.is(":visible") ? h.outerHeight(true) : 0;
    var g = k.is(":visible") ? k.outerHeight(true) : 0;
    var q = p.is(":visible") ? p.outerHeight(true) : 0;
    var r = l - b - g - q - Fai.getFrameHeight(j);
    j.css("height", r + "px");
    var n = j.find(".formMiddleCenter" + m);
    r = r - Fai.getFrameHeight(n);
    var o = j.find(".formMiddleContent" + m);
    r = r - Fai.getFrameHeight(o);
    o.css("height", r + "px");
    o.css("overflow-y", "hidden");
    if (c.hasClass("formStyle29")) {
        var i = c.find(".formTabButtonList" + m);
        if (i.length == 1) {
            var a = i.outerHeight(true);
            var d = c.find(".formTabContent" + m);
            var f = r - a - Fai.getCssInt(d, "border-top-width") - Fai.getCssInt(d, "border-bottom-width");
            if (Fai.isIE6()) {
                f += 1
            }
            d.css("height", f + "px");
            var e = c.find(".formTabCntId");
            e.css("height", f + "px")
        }
    }
    if (c.hasClass("formStyle79")) {
        c.find("#float_img_" + m).css("height", r + "px")
    }
};
...
Site.reSetSidePosition = function(b) {
    ...
    if (c == 1) {
        if (n == "floatLeftTopForms" || n == "floatLeftBottomForms") {
            ...
            b.bind("mouseenter", Site.bindSideIn1);
            b.bind("mouseleave", Site.bindSideOut1)
        } else {
            if (n == "floatRightTopForms" || n == "floatRightBottomForms") {
                ...
                b.bind("mouseenter", Site.bindSideIn2);
                b.bind("mouseleave", Site.bindSideOut2)
            } else {
                b.attr("_side", 0)
            }
        }
    ...
    if (Fai.top._manageMode) {
        ...
        },
        {
            ...
            complete: function() {
                ...
                if (c == 1) {
                    b.unbind("mouseenter", Site.bindSideIn1).unbind("mouseleave", Site.bindSideOut1).unbind("mouseenter", Site.bindSideIn2).unbind("mouseleave", Site.bindSideOut2)
                }
            }
        })
    } else {
...
Site.setAbsFormsHolder2 = function() {
    var d = function(f) {
        if (!Fai.top._manageMode && !Fai.isIE6() && !Fai.isIE7()) {
            return f
        }
        var g = Fai.top.$("#g_main").scrollTop();
        if (g <= 0) {
            return f
        }
        return f + g
    ...
    Fai.top.$("#absForms >.form").each(function() {
        var f = $(this).offset().top + $(this).outerHeight();
        f = d(f);
        if (f > c) {
            c = f
        }
    ...
    var b = Fai.top.$("#containerPlaceholder");
    var a = d(b.offset().top);
    if (c > a) {
        var e = c - a;
        if (e !== parseInt(b.height())) {
            b.css("height", e + "px");
...
Site.setAbsFormsHolder = function() {
    var c = 0;
    Fai.top.$("#absForms >.form").each(function() {
    ...
    if (c > a) {
        ...
        if (d !== b.height()) {
            b.css("height", d + "px");
            Fai.top._colOtherStyleData.y = parseInt(d)
        }
    } else {}
};
Site.setAbsTopFormsHolder = function() {
    var c = 0;
    Fai.top.$("#absTopForms >.form").each(function() {
        var e = $(this).offset().top + $(this).outerHeight();
        if (e > c) {
...
Site.checkAbsModulePosition = function(b) {
    var g = b.parent().attr("id");
    var e = b.offset().left;
    var c = b.offset().top;
    var d = b.height();
    var f = b.offset().top + d;
    if (g == "absTopForms") {
        var a = Fai.top.$("#web").offset().top;
        if (c < a) {
            c = a;
            b.offset({
                top: c,
        ...
        if (g == "absBottomForms") {
            var a = Fai.top.$("#webFooter").offset().top;
            if (c < a) {
                c = a
            }
            if (Fai.top._siteVer < 50) {
                var a = Site.getFooterBottom();
                if (c + d > a) {
                    c = a - d;
                    b.offset({
                        top: c,
                ...
                var a = Site.getFooterBottom(true);
                if (c + d > a) {
                    c = a - d;
                    b.offset({
                        top: c,
...
Site.initPage2 = function() {
    Site.manageFaiscoAd();
    Fai.top.$(".absForms >.form").each(function() {
        Site.checkAbsModulePosition($(this))
    });
    ...
    Site.bindGobalEvent("site_moduleTabSwitch",
    function(b, d) {
...
Site.initBackToTopTool = function(f, b, g) {
    if (!b) {
        return
    }
    var c = $('<div id="siteBackToTop_small_box" class="siteBackToTop_small_box" title="' + g + '"></div>');
    $("#g_main").before(c);
    var d = $("#siteBackToTop_small_box");
    var e = Fai.top.$("#g_main");
    if (!f && !Fai.isIE6()) {
        e = $(window);
        d.css("right", 5 + "px");
        d.css("bottom", 5 + "px")
...
Site.fixSiteWidth = function(b) {
    ...
    Fai.top.$(window).on("resize.fixSiteWidth",
    function() {
        a(b);
        Site.fixWebFooterHeight();
        Site.refreshDefaultBannerEdge()
    ...
    function a(c) {
        ...
        l = k;
        var j = document.documentElement ? document.documentElement.clientWidth: document.body.clientWidth,
        f = document.documentElement ? document.documentElement.scrollWidth: document.body.scrollWidth,
        g = $("#web"),
        i = $("#g_main"),
        d = 0,
        h = 0,
...
Site.initPageBindAmousedown = function() {
    ...
    top.$(top.document).delegate("div.navSubMenu a", "mousedown",
    function(f) {
        if (!a(f)) {
            return
        }
...
Site.createMemberBar = function(d, b, c) {
    if (Fai.top.$("#memberBarArea").length > 0) {
        return
    }
    var a = new Array();
    a.push("<div id='memberBarArea' class='memberBarArea g_editPanel' >");
    a.push("<div id='memberBar' class='memberBar'>");
    a.push("<div class='left'>");
    a.push("<div>");
    a.push("<span class='memberHello'>" + LS.memberPlease + "</span>");
    a.push("<a class='memberOption' href='login.jsp' >[" + LS.memberLogin + "]</a>");
    a.push("<a class='memberOption' href='signup.jsp' >[" + LS.memberReg + "]</a>");
    if (c) {
        a.push("<div id='mallCart_js' class='mallCart'>");
        a.push("<div class='mallCartItem'>");
        a.push("<span class='mallCart_icon'>&nbsp;</span>");
        a.push("<span class='mallCart_name'>" + LS.memberMallCart + "</span>");
        a.push("<span class='mallCart_proNum'>(0)</span>");
        a.push("<span class='mallCart_down'>&nbsp;</span>");
        a.push("</div>");
        a.push("<div class='mallCartPanel'>");
        a.push("<div id='mallCartList_js' class='mallCartList'>");
        a.push("<div class='mcProductList'>" + LS.memberMallCartNoProduct + "</div>");
        a.push("<div class='checkMallCartBtn'>" + LS.memberMallCartCheckBtn + "</div>");
        a.push("</div>");
        a.push("</div>");
        a.push("</div>")
    }
    if (d) {
        ...
        a.push("<a href='javascript:;' class='l_Btn'>");
        a.push("<span class='l_Ico qqLgn'></span><span>" + LS.memberQQLgn + "</span>");
        a.push("</a>")
    }
    if (b) {
        a.push("<div class='line'></div>");
        a.push("<a href='javascript:;' class='l_Btn'>");
        a.push("<span class='l_Ico wbLgn'></span><span>" + LS.memberSnLgn + "</span>");
        a.push("</a>")
    }
    a.push("</div>");
    a.push("</div>");
...
Site.imageEffect = function(a, h, d, i, e) {
    var c = $("#module" + a);
    var b = h.effType;
    if (h.effType == 1 || Fai.isNull(c)) {
        return
    }
    var f = $(c).find(".imageEffects");
    var g = $(c).find("." + d.target);
    if ($(f).length > 0) {
        $(f).remove()
    }
    $.each(g,
    function(j, k) {
        if (b > 1 && b < 6) {
            Site.createImageEffect_ImgEff(k, h, d);
            Site.bindImageEffectsCusEvent(k, d);
            if (b == 4 || b == 5) {
                if (!Fai.isNull(i)) {
                    i(k, h, d, e)
                }
            ...
            Site.createImageEffect_Img(k, h, d)
        }
    });
    $(g).bind("mouseenter mouseleave",
    function(k) {
        var j = $(this).find(".imageEffects");
        if (b < 6) {
            Site.resetImageEffect(d, this, j, b)
        } else {
            j = $(this).find("img")
        }
        if (k.type == "mouseenter") {
            Site.ImageEffectAnimate("mouseenter", j, b)
        } else {
            Site.ImageEffectAnimate("mouseleave", j, b)
        }
    })
};
Site.createImageEffect_ImgEff = function(f, e, a) {
    var d = e.effType;
    var g = [];
    b();
    c();
    function b() {
        g.push("<div class='imageEffects imageEffectsAnimate'></div>");
        var h = $(f).find("a");
        if (!Fai.isNull(h) && h.length != 0) {
            $(f).find("a").after(g)
        } else {
            $(f).find("img").after(g)
        }
    ...
    function c() {
        $(f).css("position", "relative").css("overflow", "hidden");
        if (Fai.isIE6() && a.targetParent == "photoSmallPic_table") {
            $(f).removeAttr("style")
        }
        var q = $(f).find(".imageEffects");
        var i = {
            bw: (e.borderType) ? e.borderWidth: 1,
            bs: (e.borderType) ? e.borderStyle: 1,
            bc: (e.borderType) ? e.borderColor: "#000"
        };
        var k = (i.bs == 1) ? "solid": (i.bs == 2) ? "dotted": (i.bs == 3) ? "dashed": "";
        var j = $(f).find("img");
        var m = $(j).width();
        var n = $(j).height();
        var h = $(j).css("margin-left");
        var l = $(j).position().top;
        var p = $(j).position().left;
        var o = (d == 2) ? "imageEffects_Border": (d == 3) ? "imageEffects_Magnifier": (d == 4) ? "imageEffects_FullMaks": (d == 5) ? "imageEffects_HalfMaks": "";
        h = Site.parsePixelToInt(h);
        if (d == 2) {
            m -= i.bw * 2;
            n -= i.bw * 2;
            $(q).css("border", i.bw + "px " + k + " " + i.bc)
        }
        if (d == 5) {
            n /= 3;
            $(q).removeClass("imageEffectsAnimate").addClass("imageEffectsAnimate_Half");
            $(q).css("bottom", "-" + n + "px")
        } else {
            $(q).css("top", l)
        }
        $(q).css("width", m).css("height", n + "px");
        $(q).css("left", p + h);
        $(q).addClass(o)
    }
};
Site.createImageEffect_Img = function(f, e, a) {
    var c = e.effType;
    b();
    d();
    function b() {
        ...
        if (c == 6) {
            var h = $(i).width();
            var g = $(i).height();
            $(i).attr("_initWidth", h).attr("_initHeight", g)
        }
        if (c == 7) {
            var h = $(i).width();
            $(i).css("width", h + 10 + "px")
    ...
    function d() {
        var g = (c == 6) ? "imageEffectsAnimate_Magnify": "imageEffectsAnimate_MoveLeft";
        $(f).css({
            overflow: "hidden"
        });
        $(f).find("img").addClass(g).css("margin", "0px")
    }
};
Site.ImageEffectAnimate = function(e, c, b) {
    var d = {
        hover: {},
        leave: {}
    };
    ...
    if (!Fai.isIE()) {
        ...
        if (e == "mouseenter") {
            $(c).stop().animate(d.hover, 250)
        } else {
            if (e == "mouseleave") {
                $(c).stop().animate(d.leave, 250)
            }
        }
    }
    function a(i, h) {
        if (b == 2 || b == 3 || b == 4) {
            ...
            if (b == 5) {
                $(c).css("opacity", 1);
                i.hover.bottom = 0;
                i.leave.bottom = "-" + $(c).height() + "px"
            } else {
                if (b == 6) {
                    var g = parseInt($(h).attr("_initWidth"));
                    var f = parseInt($(h).attr("_initHeight"));
                    i.hover.width = g + 50;
                    i.hover["margin-left"] = "-25px";
                    i.hover.height = f + 50;
                    i.hover["margin-top"] = "-25px";
                    i.leave.width = g;
                    i.leave["margin-left"] = "0px";
                    i.leave.height = f;
                    i.leave["margin-top"] = "0px"
                } else {
                    i.hover["margin-left"] = "-10px";
                    i.leave["margin-left"] = "0px"
                }
            }
        }
...
Site.bindImageEffectsCusEvent = function(c, b) {
    var a = {
    ...
    if (typeof a[b.targetParent] == "function") {
        a[b.targetParent](c, b)
    }
};
Site.resetImageEffect = function(b, d, i, a) {
    var e = $(d).find("img");
    var j = $(e).position().top;
    var c = $(e).position().left;
    var h = $(e).width();
    var g = $(e).height();
    var k = $(e).css("margin-left");
    var f = $(i).css("border-left-width");
    k = Site.parsePixelToInt(k);
    f = Site.parsePixelToInt(f);
    if (b.targetParent == "photoSmallPic_table") {
        $(i).css("left", c + k).css("top", j);
        $(i).css("width", h - (f * 2) + "px");
        $(i).css("height", g - (f * 2) + "px")
    }
    if (b.targetParent == "photo-container" || b.targetParent == "product-container") {
        if (a != 5) {
            $(i).css("left", c + k).css("top", j)
        } else {
            $(i).css("left", c + k)
...
Site.parsePixelToInt = function(a) {
    var b;
